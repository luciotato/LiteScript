        ...
        switch (url_parts.pathname) {
            ...
            */
            ...
            case '/dataserver': { //?q=select * from dual
                ...
                return true; //handled
            }// end case '/dataserver': //?q=select * from dual
            ...
            case '/do': { //?cmd=command
                switch(url_parts.query.cmd){
                    case "ls": {
                        ...
                        fs.readdir(wd,
                        else if (done:(err,files)) {
                            if (err) {
                                response.end(err.message);
                            }
                            else {
                                var onlyFiles = files.filter(function(item,index){return (item[0] !=  '.')});
                                response.end(JSON.stringify(onlyFiles));
                            } //end if
                        }// end when done:(err,files)
                 ...
                 // end switch(url_parts.query.cmd){
                }
                break;
            }// end case '/do': //?cmd=command

            default: {
                return false; // no manejado, que busque un static file
         // end switch (url_parts.pathname) {
        }
    }// end try
...
function insertToken(response) {
    ...
    function (err,data) {
        if (err ) {
            if (response.count<10 && err.message.indexOf( 'duplicated')>=0 ) {
