...
//    syncEditors([CompareLite_ed, CompareJs_ed],false);
    ...
    }// end httpGet('samples/'+fname+'.lite.js',...
...
function addLines(lineArray) {
    ...
    if (typeof lineArray === 'string') { //called with a string as parameter
        var a=[];
        a[0]=lineArray;
        lineArray=a;
    } //end if
...
*/
    ...
    }// end for var n=0 to aceEditors.length
...
function addRun(liteLines,jsLines) {
    ...
    if (liteLines === 'Array') {
        ...
        for (n=liteLines.length ; n <= jsLines.length ; n++) {
            liteLines[n]="";
        } //end for

        for (n=jsLines.length ; n <= liteLines.length ; n++) {
            jsLines[n]="";
        } //end for
