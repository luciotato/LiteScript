...
any Parser_pushAt(DEFAULT_ARGUMENTS); //forward declare
    ...
    static propIndex_t Parser_DynBuffer_PROPS[] = {
    used_
    , buf_
    };
...
//First, remember filename (for error reporting) 
          ...
          PROP(interfaceMode_,this) = any_number(!__is(METHOD(indexOf_,filename)(filename,1,(any_arr){any_LTR(".interface.")}),any_number(-1)));
...
//except for title-keywords 
                ...
                //else
                    ...
                    if (__is(indent,any_number(0)) && __is(METHOD(charAt_,line)(line,1,(any_arr){any_number(0)}),any_LTR("#")))  { //starts on column 1, with a '#'
...
//After applying rules: if we're in a Code Block, is CODE, else is a COMMENT
                ...
                if (_anyToBool(inCodeBlock))  {
                    ...
                    if (_anyToBool((_anyToBool(__or1=METHOD(startsWith_,line)(line,1,(any_arr){any_LTR("#")}))? __or1 : METHOD(startsWith_,line)(line,1,(any_arr){any_LTR("//")}))))  {// # CODE by indent, but all commented
...
//Create infoLine, with computed indent, text, and source code line num reference 
            ...
            var infoLine = new(Parser_InfoLine,5,(any_arr){this, type, indent, line, sourceLineNum});
            //infoLine.dump() # debug
...
//check for title-keywords: e.g.: `### Class MyClass`, `### export Function compile(sourceLines:string array)`
        ...
        if (_anyToBool(__call(trim_,__call(replaceAll_,ITEM(0,words),2,(any_arr){any_LTR("#"), any_LTR(" ")}),0,NULL))) {return undefined;};
        ...
        while(_anyToNumber(inx) < _length(words)){
            ...
            if (_anyToBool((w=ITEM(_anyToNumber(inx),words))))  { //skip empty items
                ...
                if (_anyToNumber((posParen=METHOD(indexOf_,w)(w,1,(any_arr){any_LTR("(")}))) != -1)  {
        ...
        if (_anyToNumber(countAdj) > 1 && !_anyToBool(countSust)) {METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(2,any_LTR("MarkDown Title-keyword, expected a sustantive: "), METHOD(join_,sustantives)(sustantives,0,NULL))});};
        ...
        if (_anyToBool(countSust))  {
            ...
            ITEM(0,words) = __call(replaceAll_,ITEM(0,words),2,(any_arr){any_LTR("#"), any_LTR(" ")}); //replace # by ' '

            //return words.join(' ') // re-join
            return METHOD(join_,words)(words,1,(any_arr){any_LTR(" ")}); // re-join
        };
     return undefined;
     }
...
//Now, after processing all lines, we tokenize each CODE line
        ...
        for(int item__inx=0 ; item__inx<_list13.value.arr->length ; item__inx++){item=ITEM(item__inx,_list13);
            ...
            //catch err
                ...
                throw(new(ControlledError,1,(any_arr){_concatAny(5,PROP(filename_,this), any_LTR(":"), PROP(sourceLineNum_,item), any_LTR(":1 "), PROP(message_,err))}));
            };
...
//Here we search for simple macros as __DATE__, __TIME__ , __TMESTAMP__
        ...
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
            ...
            line = METHOD(replaceAll_,line)(line,2,(any_arr){_concatAny(3,any_LTR("__"), macro, any_LTR("__")), value});
...
//get source line, replace TAB with 4 spaces, remove trailing withespace and remove CR
        ...
        PROP(line_,this) = __call(replaceAll_,__call(trimRight_,__call(replaceAll_,ITEM(_anyToNumber(PROP(sourceLineNum_,this)),PROP(lines_,this)),2,(any_arr){any_LTR("\t"), any_LTR("    ")}),0,NULL),2,(any_arr){any_LTR("\r"), any_EMPTY_STR});
...
//Get section between """ and """
        ...
        var result = new(Parser_MultilineSection,4,(any_arr){this, line, any_LTR("\"\"\""), any_LTR("\"\"\"")});
        //if result.section
...
//Now we should escape internal d-quotes, but only *outside* string interpolation expressions
          ...
          for(int inx=0 ; inx<_list17.value.arr->length ; inx++){item=ITEM(inx,_list17);
              ...
              if (__is(METHOD(charAt_,item)(item,1,(any_arr){any_number(0)}),any_LTR("\"")))  { //a string part
                  ...
                  ITEM(inx,parsed) = METHOD(replaceAll_,item)(item,2,(any_arr){any_LTR("\""), any_LTR("\\\"")});// #store with *escaped* internal d-quotes
              }
              //else
                  ...
                  ITEM(inx,parsed) = _concatAny(4,PROP(stringInterpolationChar_,this), any_LTR("{"), item, any_LTR("}"));
              };
          }};// end for each in parsed

          //#re-join & re.enclose in quotes
          //line = parsed.join("").quoted('"') 
          line = __call(quoted_,METHOD(join_,parsed)(parsed,1,(any_arr){any_EMPTY_STR}),1,(any_arr){any_LTR("\"")});
          //line = "#{result.pre} #{line}#{result.post}" #add pre & post
          line = _concatAny(4,PROP(pre_,result), any_LTR(" "), line, PROP(post_,result));// #add pre & post
        };
...
//This method handles multiline comments: `/*` `*/` 
        ...
        if (__is(any_number(_length(PROP(section_,result))),any_number(1)))  {// # just one line
          ...
          line = _concatAny(5,PROP(pre_,result), any_LTR(" "), PROP(post_,result), any_LTR("//"), ITEM(0,PROP(section_,result)));
        ...
        //else 
          ...
          if (_anyToBool(__call(trim_,PROP(post_,result),0,NULL)))  {
              ...
              logger_warning(undefined,1,(any_arr){_concatAny(4,PROP(filename_,this), any_LTR(":"), PROP(sourceLineNum_,this), any_LTR(":1. Do not add text on the same line after `*/`. Indent is not clear"))});
...
//ifdef, #ifndef, #else and #endif should be the first thing on the line
        ...
        if (__is(METHOD(indexOf_,line)(line,1,(any_arr){any_LTR("#endif")}),any_number(0))) {METHOD(throwErr_,this)(this,1,(any_arr){any_LTR("found \"#endif\" without \"#ifdef\"")});};
        //if line.indexOf("#else") is 0, .throwErr 'found "#else" without "#ifdef"'
        if (__is(METHOD(indexOf_,line)(line,1,(any_arr){any_LTR("#else")}),any_number(0))) {METHOD(throwErr_,this)(this,1,(any_arr){any_LTR("found \"#else\" without \"#ifdef\"")});};
        ...
        var startRef = _concatAny(2,any_LTR("while processing #ifdef started on line "), startSourceLine);

        //words = line.slice(pos).split(' ')
        words = __call(split_,METHOD(slice_,line)(line,1,(any_arr){pos}),1,(any_arr){any_LTR(" ")});
        ...
        METHOD(replaceSourceLine_,this)(this,1,(any_arr){__call(replaceAll_,PROP(line_,this),2,(any_arr){any_LTR("#if"), any_LTR("//if")})});
        ...
        do{
            ...
            if (!_anyToBool(METHOD(nextSourceLine_,this)(this,0,NULL))) {METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(2,any_LTR("EOF "), startRef)});};
            //line = .line
            ...
            if (_anyToNumber((indent=METHOD(countSpaces_,line)(line,0,NULL))) >= 0)  {
                ...
                if (__is(METHOD(charAt_,line)(line,1,(any_arr){any_number(0)}),any_LTR("#")) && !__is(METHOD(charAt_,line)(line,1,(any_arr){any_number(1)}),any_LTR("#")))  { //expected: "#else, #endif #end if"
                    //words = line.split(' ')
                    words = METHOD(split_,line)(line,1,(any_arr){any_LTR(" ")});
                    //case words.tryGet(0)
                    
                        //when '#else'
                    if (__is(METHOD(tryGet_,words)(words,1,(any_arr){any_number(0)}),any_LTR("#else"))){
                            //.replaceSourceLine .line.replaceAll("#else","//else")
                            METHOD(replaceSourceLine_,this)(this,1,(any_arr){__call(replaceAll_,PROP(line_,this),2,(any_arr){any_LTR("#else"), any_LTR("//else")})});
                            //defValue = not defValue
                            defValue = any_number(!(_anyToBool(defValue)));
                    
                    }
                        //when "#end"
                    else if (__is(METHOD(tryGet_,words)(words,1,(any_arr){any_number(0)}),any_LTR("#end"))){
                            //if words.tryGet(1) isnt 'if', .throwErr "expected '#end if', read '#{line}' #{startRef}"
                            if (!__is(METHOD(tryGet_,words)(words,1,(any_arr){any_number(1)}),any_LTR("if"))) {METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(4,any_LTR("expected '#end if', read '"), line, any_LTR("' "), startRef)});};
                            //endFound = true
                            endFound = true;
                    
                    }
                        //when "#endif"
                    else if (__is(METHOD(tryGet_,words)(words,1,(any_arr){any_number(0)}),any_LTR("#endif"))){
                            //endFound = true
                            endFound = true;
                        ...
                        //else
                            ...
                            METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(4,any_LTR("expected '#else/#end if', read '"), line, any_LTR("' "), startRef)});
                    };
                    //end case
                ...
                //else
                    ...
                    if (!(_anyToBool(defValue)) && _anyToBool(line)) {METHOD(replaceSourceLine_,this)(this,1,(any_arr){_concatAny(3,String_spaces(undefined,1,(any_arr){indent}), any_LTR("//"), line)});};
                };
                //end if
        ...
        METHOD(replaceSourceLine_,this)(this,1,(any_arr){__call(replaceAll_,PROP(line_,this),2,(any_arr){any_LTR("#end"), any_LTR("//end")})});
...
//Save current pos, and get next token
        ...
        logger_debug(undefined,3,(any_arr){any_LTR(">>>ADVANCE"), _concatAny(6,PROP(sourceLineNum_,this), any_LTR(":"), (_anyToBool(__or2=PROP(column_,PROP(token_,this)))? __or2 : any_number(0)), any_LTR(" ["), PROP(index_,this), any_LTR("]")), __call(toString_,PROP(token_,this),0,NULL)});
...
//#### method returnToken()
     any Parser_Lexer_returnToken(DEFAULT_ARGUMENTS){
        ...
        logger_debug(undefined,4,(any_arr){any_LTR("<< Returned:"), __call(toString_,PROP(token_,this),0,NULL), any_LTR("line"), PROP(sourceLineNum_,this)});
     return undefined;
     }
...
//**throwErr** add lexer position and emit error (abort compilation)
        ...
        logger_throwControlled(undefined,1,(any_arr){_concatAny(3,METHOD(posToString_,this)(this,0,NULL), any_LTR(" "), msg)});
     return undefined;
     }
...
//split expressions
        ...
        while(TRUE){
            ...
            delimiterPos = METHOD(indexOf_,s)(s,2,(any_arr){_concatAny(2,delimiter, any_LTR("{")), lastDelimiterPos});
            ...
            closerPos = String_findMatchingPair(undefined,3,(any_arr){s, start, any_LTR("}")});

            //if closerPos<0
            if (_anyToNumber(closerPos) < 0)  {
                //.throwErr "unmatched '#{delimiter}{' at string: #{text}"
                METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(4,any_LTR("unmatched '"), delimiter, any_LTR("{' at string: "), text)});
            };
            ...
            var singleUnit = PMREX_whileRanges(undefined,2,(any_arr){item, any_LTR("A-Za-z0-9_$.")});
            //if item isnt singleUnit, item = '(#{item})';
            if (!__is(item,singleUnit)) {item = _concatAny(3,any_LTR("("), item, any_LTR(")"));};
...
//-the column in the source line in which the token appears
    ...
    //class Token
        ...
        any Parser_Token_toString(DEFAULT_ARGUMENTS){
            ...
            //---------
            //return "'#{.value}'(#{.type})"
            return _concatAny(5,any_LTR("'"), PROP(value_,this), any_LTR("'("), PROP(type_,this), any_LTR(")"));
        return undefined;
        }
    ...
    //class InfoLine
      ...
      any Parser_InfoLine_dump(DEFAULT_ARGUMENTS){
        ...
        if (__is(PROP(type_,this),Parser_LineTypes_BLANK))  {
          ...
          logger_debug(undefined,2,(any_arr){PROP(sourceLineNum_,this), any_LTR("(BLANK)")});
          //return
          return undefined;
        };
        ...
        logger_debug(undefined,3,(any_arr){PROP(sourceLineNum_,this), _concatAny(4,PROP(indent_,this), any_LTR("("), type, any_LTR(")")), PROP(text_,this)});
        //if .tokens
        if (_anyToBool(PROP(tokens_,this)))  {
            //logger.debug('   ',.tokens.join(' '))
            logger_debug(undefined,2,(any_arr){any_LTR("   "), __call(join_,PROP(tokens_,this),1,(any_arr){any_LTR(" ")})});
            //logger.debug()
...
//If there was no match, this is a bad token and we will abort compilation here.
            ...
            if (!_anyToBool(token))  {
                ...
                msg = _concatAny(8,any_LTR("("), PROP(filename_,lexer), any_LTR(":"), PROP(sourceLineNum_,this), any_LTR(":"), any_number(_anyToNumber(colInx) + 1), any_LTR(") Tokenize patterns: invalid token: "), chunk);
                //logger.error msg 
                ...
                while(_length(errPosString) < _anyToNumber(colInx)){
                    ...
                    errPosString = _concatAny(2,errPosString, any_LTR(" "));
                };// end loop

                //logger.error code
                logger_error(undefined,1,(any_arr){code});
                //logger.error '#{errPosString}^'
                logger_error(undefined,1,(any_arr){_concatAny(2,errPosString, any_LTR("^"))});
...
//with a call to core function "concat"
                ...
                ////ifdef PROD_C
                    ...
                    var composed = new(Parser_InfoLine,5,(any_arr){lexer, Parser_LineTypes_CODE, PROP(column_,token), _concatAny(3,any_LTR("_concatAny("), METHOD(join_,parsed)(parsed,1,(any_arr){any_LTR(",")}), any_LTR(")")), PROP(sourceLineNum_,this)});
...
//`lexer options literal (map|object)`
        ...
        if (__is(METHOD(tryGet_,words)(words,1,(any_arr){any_number(0)}),any_LTR("lexer")) && __is(METHOD(tryGet_,words)(words,1,(any_arr){any_number(1)}),any_LTR("options")))  {
            ...
            if (__is(__call(join_,METHOD(slice_,words)(words,2,(any_arr){any_number(2), any_number(5)}),1,(any_arr){any_LTR(" ")}),any_LTR("string interpolation char")))  {
                //var ch:string
                var ch = undefined;
                //if words.tryGet(5) into ch is 'is' then ch = words.tryGet(6) #get it (skip optional 'is')
                if (__is((ch=METHOD(tryGet_,words)(words,1,(any_arr){any_number(5)})),any_LTR("is"))) {ch = METHOD(tryGet_,words)(words,1,(any_arr){any_number(6)});};
                //if ch.charAt(0) in ['"',"'"], ch = ch.slice(1,-1) #optionally quoted, remove quotes
                if (__in(METHOD(charAt_,ch)(ch,1,(any_arr){any_number(0)}),2,(any_arr){any_LTR("\""), any_LTR("'")})) {ch = METHOD(slice_,ch)(ch,2,(any_arr){any_number(1), any_number(-1)});};
            ...
            else if (__is(__call(join_,METHOD(slice_,words)(words,2,(any_arr){any_number(2), any_number(5)}),1,(any_arr){any_LTR(" ")}),any_LTR("object literal is")))  {
...
//Comment lines, start with # or //
            ...
            if (_anyToBool((_anyToBool(__or4=METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR("#")}))? __or4 : METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR("//")}))))  {
                //return new Token('COMMENT',chunk)
                return new(Parser_Token,2,(any_arr){any_LTR("COMMENT"), chunk});
            };
...
//They're not considered 'operators' since they do no introduce a new operand, ++ and -- are "modifiers" for a variable reference.
  ...
  //['PUNCT',/^[\(\)\[\]\;\,\.\{\}]/],
            ...
            if (CALL1(indexOf_,any_LTR("()[]{};,."),METHOD(charAt_,chunk)(chunk,1,(any_arr){any_number(0)})).value.number>=0)  {
                //return new Token('PUNCT',chunk.slice(0,1))
                return new(Parser_Token,2,(any_arr){any_LTR("PUNCT"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(1)})});
            };
            //if chunk.slice(0,2) in ["++","--","->"]
            if (__in(METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(2)}),3,(any_arr){any_LTR("++"), any_LTR("--"), any_LTR("->")}))  {
                //return new Token('PUNCT',chunk.slice(0,2))
                return new(Parser_Token,2,(any_arr){any_LTR("PUNCT"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(2)})});
            };
...
//We recognize ' [' (space+bracket) to be able to diferntiate: 'myFunc [x]' and 'myFunc[x]'
  ...
  //['WHITESPACE',/^[\f\r\t\v\u00A0\u2028\u2029 ]+/],
            ...
            if (_anyToBool(METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR(" .")})))  {
                ...
                return new(Parser_Token,2,(any_arr){any_LTR("SPACE_DOT"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(2)})});
            };
            //if chunk.startsWith(" [")
            if (_anyToBool(METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR(" [")})))  {
                //return new Token('SPACE_BRACKET',chunk.slice(0,2))
                return new(Parser_Token,2,(any_arr){any_LTR("SPACE_BRACKET"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(2)})});
            };
            //if PMREX.whileRanges(chunk," \t\r") into var whiteSpace
            var whiteSpace=undefined;
            if (_anyToBool((whiteSpace=PMREX_whileRanges(undefined,2,(any_arr){chunk, any_LTR(" \t\r")}))))  {
                //if chunk.charAt(whiteSpace.length) in '.[', whiteSpace=whiteSpace.slice(0,-1) //allow recognition of SPACE_DOT and SPACE_BRACKET
                if (CALL1(indexOf_,any_LTR(".["),METHOD(charAt_,chunk)(chunk,1,(any_arr){any_number(_length(whiteSpace))})).value.number>=0) {whiteSpace = METHOD(slice_,whiteSpace)(whiteSpace,2,(any_arr){any_number(0), any_number(-1)});};
                //return new Token('WHITESPACE',whiteSpace)
                return new(Parser_Token,2,(any_arr){any_LTR("WHITESPACE"), whiteSpace});
            };
...
//Strings can be either single or double quoted.
  ...
  //['STRING', /^"(?:[^"\\]|\\.)*"/],
            ...
            //if chunk.startsWith("'") or chunk.startsWith('"')
            if (_anyToBool((_anyToBool(__or5=METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR("'")}))? __or5 : METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR("\"")}))))  {
                ...
                return new(Parser_Token,2,(any_arr){any_LTR("STRING"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(1 + _length(quotedContent) + 1)})}); //include quotes
            };
...
//In LiteScript, assignment is a *statement* not a *expression*
  ...
  //['ASSIGN',/^[\+\-\*\/\&]=/ ], # = += -= *= /= &=
            ...
            if (_anyToBool(METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR("=")})))  {
                ...
                return new(Parser_Token,2,(any_arr){any_LTR("ASSIGN"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(1)})});
            };
            //if chunk.charAt(0) in "+-*/&" and chunk.charAt(1) is "=" 
            if (CALL1(indexOf_,any_LTR("+-*/&"),METHOD(charAt_,chunk)(chunk,1,(any_arr){any_number(0)})).value.number>=0 && __is(METHOD(charAt_,chunk)(chunk,1,(any_arr){any_number(1)}),any_LTR("=")))  {
                //return new Token('ASSIGN',chunk.slice(0,2))
                return new(Parser_Token,2,(any_arr){any_LTR("ASSIGN"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(2)})});
            };
...
//Regex tokens are regular expressions. The javascript producer, just passes the raw regex to JavaScript.
  ...
  //['REGEX', /^(\/(?![\s=])[^[\/\n\\]*(?:(?:\\[\s\S]|\[[^\]\n\\]*(?:\\[\s\S][^\]\n\\]*)*])[^[\/\n\\]*)*\/)([imgy]{0,4})(?!\w)/],
            ...
            if (_anyToBool(METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR("/")})))  {
                ...
                return new(Parser_Token,2,(any_arr){any_LTR("REGEX"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(_length(regexpContents) + 2)})}); //include quote-chars: / & /
            };
...
//that sits between *two* operands in a `Expressions`. 
  ...
  //['OPER', /^(is|isnt|not|and|but|into|like|or|in|into|instance|instanceof|has|hasnt|bitand|bitor)\b/],
            ...
            if (__is(METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(3)}),any_LTR("!==")))  {
                //return new Token('OPER',chunk.slice(0,3))
                return new(Parser_Token,2,(any_arr){any_LTR("OPER"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(3)})});
            };

            //if "|#{chunk.slice(0,2)}|" in "|<>|>=|<=|>>|<<|!=|"
            if (CALL1(indexOf_,any_LTR("|<>|>=|<=|>>|<<|!=|"),_concatAny(3,any_LTR("|"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(2)}), any_LTR("|"))).value.number>=0)  {
                //return new Token('OPER',chunk.slice(0,2))
                return new(Parser_Token,2,(any_arr){any_LTR("OPER"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(2)})});
            };

            //if chunk.charAt(0) in "><+-*/%&~^|?:" 
            if (CALL1(indexOf_,any_LTR("><+-*/%&~^|?:"),METHOD(charAt_,chunk)(chunk,1,(any_arr){any_number(0)})).value.number>=0)  {
                //return new Token('OPER',chunk.slice(0,1))
                return new(Parser_Token,2,(any_arr){any_LTR("OPER"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(1)})});
            };
...
//As in js, all numbers are floating point.
  ...
  //['NUMBER',/^[0-9]+(\.[0-9]+)?(e[+-]?[0-9]+)?/i],
            ...
            if (_anyToBool(METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR("0x")})))  {
                ...
                var hexContent = PMREX_whileRanges(undefined,2,(any_arr){METHOD(slice_,chunk)(chunk,1,(any_arr){any_number(2)}), any_LTR("a-fA-F0-9")});
                //return new Token('NUMBER',chunk.slice(0, hexContent.length+2)) //include 0x
                return new(Parser_Token,2,(any_arr){any_LTR("NUMBER"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(_length(hexContent) + 2)})}); //include 0x
            };
            ...
            if (_anyToBool((numberDigits=PMREX_whileRanges(undefined,2,(any_arr){chunk, any_LTR("0-9")}))))  {
                ...
                if (__is(METHOD(charAt_,chunk)(chunk,1,(any_arr){any_number(0)}),any_LTR(".")))  {
                    //decPoint = '.'
                    decPoint = any_LTR(".");
                    //chunk=chunk.slice(1)
                    ...
                    decimalPart = PMREX_whileRanges(undefined,2,(any_arr){chunk, any_LTR("0-9")});
                ...
                if (__is(METHOD(charAt_,chunk)(chunk,1,(any_arr){any_number(0)}),any_LTR("e")))  {
                    //expE = 'e'
                    expE = any_LTR("e");
                    //chunk=chunk.slice(1)
                    chunk = METHOD(slice_,chunk)(chunk,1,(any_arr){any_number(1)});
                    ...
                    exponent = PMREX_whileRanges(undefined,2,(any_arr){chunk, any_LTR("0-9")});
                ...
                return new(Parser_Token,2,(any_arr){any_LTR("NUMBER"), _concatAny(5,numberDigits, decPoint, decimalPart, expE, exponent)});
            };
...
//(Note: we recognized numbers above)
            ...
            if (_anyToBool((identifier=PMREX_whileRanges(undefined,2,(any_arr){chunk, any_LTR("A-Za-z0-9\x7F-\xFF$_")}))))  {

                //if "|#{identifier}|" in "|is|isnt|not|and|but|into|like|or|in|into|instance|instanceof|has|hasnt|bitand|bitor|"
                if (CALL1(indexOf_,any_LTR("|is|isnt|not|and|but|into|like|or|in|into|instance|instanceof|has|hasnt|bitand|bitor|"),_concatAny(3,any_LTR("|"), identifier, any_LTR("|"))).value.number>=0)  {
                    //return new Token('OPER',identifier)
                    return new(Parser_Token,2,(any_arr){any_LTR("OPER"), identifier});
                };

                //return new Token('IDENTIFIER',identifier)
                return new(Parser_Token,2,(any_arr){any_LTR("IDENTIFIER"), identifier});
            };
      return undefined;
      }
...
//### helper class LexerPos
      ...
      any Parser_LexerPos_toString(DEFAULT_ARGUMENTS){
        ...
        return _concatAny(5,PROP(filename_,PROP(lexer_,this)), any_LTR(":"), PROP(sourceLineNum_,this), any_LTR(":"), any_number(_anyToNumber((_anyToBool(__or6=PROP(column_,PROP(token_,this)))? __or6 : any_number(0))) + 1));
      return undefined;
      }
...
//ok, found startCode, initialize
        ...
        logger_debug(undefined,2,(any_arr){any_LTR("**** START MULTILINE "), startCode});
...
//read lines until endCode is found
        ...
        while(!(_anyToNumber((endCol=METHOD(indexOf_,line)(line,1,(any_arr){endCode}))) >= 0)){
            ...
            if (!_anyToBool(METHOD(nextSourceLine_,lexer)(lexer,0,NULL)))  {
                ...
                METHOD(sayErr_,lexer)(lexer,1,(any_arr){_concatAny(9,any_LTR("EOF while processing multiline "), startCode, any_LTR(" (started on "), PROP(filename_,lexer), any_LTR(":"), startSourceLine, any_LTR(":"), startCol, any_LTR(")"))});
                //return
                return ;
            };
          ...
          if (_anyToBool(PROP(currLine_,this)))  {
              ...
              if (_anyToBool(PROP(fileMode_,this)))  {
                  ...
                  if (!_anyToBool(ITEM(_anyToNumber(PROP(header_,this)),PROP(fileIsOpen_,this))))  {
                      ...
                      ITEM(_anyToNumber(PROP(header_,this)),PROP(fHandles_,this)) = fs_openSync(undefined,2,(any_arr){filename, any_LTR("w")});
...
//--------------
    ...
    //auto Parser_DynBuffer_newFromObject
    inline any Parser_DynBuffer_newFromObject(DEFAULT_ARGUMENTS){
        return _newFromObject(Parser_DynBuffer,argc,arguments);
    }
        ////else
        ////if .sourceMap
            ////declare on mark
...
//Like node.js Buffer, but auto-extends if required
        ...
        any Parser_DynBuffer_saveLine(DEFAULT_ARGUMENTS){
          ...
          //fs.writeSync fd, "\n"
          fs_writeSync(undefined,2,(any_arr){fd, any_LTR("\n")});
        return undefined;
        }
...
void Parser__moduleInit(void){
        ...
        Parser_DynBuffer =_newClass("Parser_DynBuffer", Parser_DynBuffer__init, sizeof(struct Parser_DynBuffer_s), Object);
