...
//with the class, or match the token type to the string.
       ...
       for( var searched__inx=0,searched=undefined; searched__inx<arguments.length; searched__inx++){searched=arguments[searched__inx];
...
//                  #store closing info as soft-error to enhance error reporting
                 this.lexer.softError = new Error("" + (this.lexer.posToString()) + " indent decreased: close block indent " + (blockIndent) + " started at line " + (startLine));
                 //break #end of list
                 break;// #end of list
           };
...
//and syntax error reporting
       ...
       var msg = [];
       //for each i in args
       for( var i__inx=0,i=undefined; i__inx<args.length; i__inx++){i=args[i__inx];
...
//It evaluates and output its arguments. uses ASTBase.prototype.outCode
       ...
       for( var item__inx=0,item=undefined; item__inx<arguments.length; item__inx++){item=arguments[item__inx];
...
//if it is the first thing in the line, out indentation
         ...
         //if not .outCode.currLine, .outCode.out String.spaces(.indent-1)
...
//              declare on item pre,post,separator
             ...
             for( var inx=0,listItem=undefined; inx<item.CSL.length; inx++){listItem=item.CSL[inx];
...
//              # prepend // if necessary
             //if not item.COMMENT.startsWith("//"), .outCode.out "//"
           ...
           //.outCode.out item.toString() # try item.toString()
...
//out as comment
       ...
       if (preComment || !(line.text.startsWith("//"))) {
           prepend = "//"};
       //if no .outCode.currLine, prepend=String.spaces(line.indent)+prepend
       if (!this.outCode.currLine) {
           prepend = String.spaces(line.indent) + prepend};
       //if preComment or line.text, .outCode.out prepend+preComment+line.text
       if (preComment || line.text) {
           this.outCode.out(prepend + preComment + line.text)};
       //.outCode.startNewLine()
       this.outCode.startNewLine();

       this.outCode.lastOutCommentLine = lineInx;
    };
    ...
    ASTBase.prototype.outLinesAsComment = function(fromLine, toLine){
       ...
       //if .outCode.currLine and .outCode.currLine.trim()
...
//      declare valid .lexer.infoLines
     ...
     //default .outCode.lastOutCommentLine = -1
     if(this.outCode.lastOutCommentLine===undefined) this.outCode.lastOutCommentLine=-1;

//find comment lines in the previous lines of code.

     var preInx = inx;
     //while preInx and preInx>.outCode.lastOutCommentLine
     while(preInx && preInx > this.outCode.lastOutCommentLine){
         //preInx--
         preInx--;
...
//        #    indent,text,tokens:array
    ...
    ASTBase.prototype.addSourceMap = function(){
       ...
       //do nothing #disabled in this version. enabled in v0.5
       null;// #disabled in this version. enabled in v0.5
    };
...
//recurse on this properties and Arrays (exclude 'parent' and 'importedModule')
     ...
     for ( var name in this)if (this.hasOwnProperty(name))if(['parent', 'importedModule', 'requireCallNodes', 'exportDefault'].indexOf(name)===-1){
             ...
             if (this[name] instanceof ASTBase) {
                 ...
                 for( var item__inx=0,item=undefined; item__inx<this[name].length; item__inx++){item=this[name][item__inx];
                 if(item instanceof ASTBase){
...
//      lastOutCommentLine
    ...
    OutCode.prototype.addSourceAsComment=true;
...
//send the current line
         ...
         if (this.lineNum > 1 || this.currLine) {
