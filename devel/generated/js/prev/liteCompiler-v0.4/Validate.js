...
//        declare valid project.moduleCache
       ...
       for ( var fname in project.moduleCache)if (project.moduleCache.hasOwnProperty(fname)){
         ...
         for( var node__inx=0,node=undefined; node__inx<moduleNode.requireCallNodes.length; node__inx++){node=moduleNode.requireCallNodes[node__inx];
...
//Repeat until no conversions can be made.
       ...
       while(totalConverted < NameDeclaration.allOfThem.length){
           ...
           for( var nameDecl__inx=0,nameDecl=undefined; nameDecl__inx<NameDeclaration.allOfThem.length; nameDecl__inx++){nameDecl=NameDeclaration.allOfThem[nameDecl__inx];
...
//Also to load the global scope with built-in objects
       ...
       if (obj instanceof Object || obj === Object.prototype) {
           ...
           var list__1=Object.getOwnPropertyNames(obj);
           for( var prop__inx=0,prop=undefined; prop__inx<list__1.length; prop__inx++){prop=list__1[prop__inx];
           if(prop !== '__proto__'){
...
//                    declare valid Object.hasOwnProperty.call
                   ...
                   if (prop !== 'constructor') {
                       ...
                       if (prop === 'prototype' || (typeof obj[prop] === 'function' && obj[prop].hasOwnProperty('prototype') && !(this.isInParents(prop)))) {
...
//            declare type:Grammar.VariableRef
           ...
           if ((classFN=type.tryGetReference())) {
             ...
             if (!classFN.members['prototype']) {
               //.sayErr "TYPE: '#{Grammar.VariableRef}' has no prototype"
...
//Add to parentModule.exports, but *preserve parent*
     ...
     //if asDefault #replaces module.exports
     if (asDefault) {// #replaces module.exports
...
//        #just add to actual exports, but preserve parent
       ...
       //parentModule.exports.addMember(exportedNameDecl)
       parentModule.exports.addMember(exportedNameDecl);
...
//add parameters to function's scope
     ...
     if (this.paramsDeclarations) {
       ...
       for( var varDecl__inx=0,varDecl=undefined; varDecl__inx<this.paramsDeclarations.length; varDecl__inx++){varDecl=this.paramsDeclarations[varDecl__inx];
...
//now check each accessor
       ...
       for( var index=0,ac=undefined; index<varRef.accessors.length; index++){ac=varRef.accessors[index];
...
//declare var:type. alter on the fly scope var "types"
     ...
     if (this.specifier === 'types') {
         ...
         for( var ac__inx=0,ac=undefined; ac__inx<this.varRef.accessors.length; ac__inx++){ac=this.varRef.accessors[ac__inx];
