...
environment = {
    ...
    /*  fileInfo = {
            ...
            hasPath: (importParameter[0] in ['.',path.delimiter])
        }
    */    
        var fileInfo =  {
        ...
        fileInfo.hasPath = (importParameter[0]===path.delimiter||importParameter[0]==='.'); 
    ...
    /*  fileInfo = {
            ...
            isCore: true if it's a core module like 'fs' or 'path'
    ...
    */
        ...
        if (!fileInfo.hasPath && !fileInfo.extension && environment.isBuiltInModule(fileInfo.moduleName)){
            fileInfo.isCore = true;
        ...
        if (!fileInfo.extension || fileInfo.extension==='.lite' || fileInfo.extension==='.md') {
            ...
            var search;
            if (fileInfo.hasPath)
                search = [path.resolve(basePath,fileInfo.dirname,fileInfo.moduleName)]
            else
            //search in:
                search = [
                    path.resolve(basePath, importingModuleDirName, fileInfo.dirname, fileInfo.moduleName)
                    ,path.resolve(basePath, importingModuleDirName, 'node_modules', fileInfo.moduleName)
                    ]
            ;
        ...
        fileInfo.outRelFilename = path.relative(basePath, fileInfo.outFilename);
    ...
    ,getGlobalObject: function (name){
        try {
            return global[name];
        }
        catch(e){
            console.error("Environment.getGlobalObject '"+name+"'");
