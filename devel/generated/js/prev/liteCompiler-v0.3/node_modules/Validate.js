...
//if node is Grammar.VariableDecl, it's ImportStatement
             ...
             if (node instanceof Grammar.VariableDecl) {
...
//Repeat until no conversions can be made.
       ...
       while(totalConverted < NameDeclaration.allOfThem.length){
         ...
         for ( var nameDecl__inx=0,nameDecl=undefined; nameDecl__inx<NameDeclaration.allOfThem.length; nameDecl__inx++) {
...
//For all modules, for each node, if the specific AST node has methodName, call it
       ...
       for ( var filename in project.moduleCache) {
...
//      properties nameDecl
   ...
   //append to class Grammar.ImportStatement ###
    ...
    //method declare()  # pass 1
    Grammar.ImportStatement.prototype.declare = function(){// # pass 1
       //for each varDecl in me.list
       for ( var varDecl__inx=0,varDecl=undefined; varDecl__inx<this.list.length; varDecl__inx++) {
           var varDecl=this.list[varDecl__inx];
           //varDecl.declareInScope
...
//add parameters to function's scope
     ...
     if (this.paramsDeclarations) {
       ...
       for ( var varDecl__inx=0,varDecl=undefined; varDecl__inx<this.paramsDeclarations.length; varDecl__inx++) {
...
//        declare valid me.variant.mainVarType
       ...
       this.mainNameDecl = this.addToScope(this.variant.mainVar, {type: this.variant.mainVarType});
    };
...
//now check each accessor
       ...
       for ( var index=0,ac=undefined; index<varRef.accessors.length; index++) {
...
//if we do, set type for lvalue (unless is "null" or "undefined", they destroy type info)
     ...
     if (rvalueType instanceof NameDeclaration && ["undefined", "null"].indexOf(rvalueType.name)===-1) {
...
//        declare valid me.root.getResultType
       ...
       return this.root.getResultType();// # me.root is Grammar.Oper or Grammar.Operand
    };
...
//if we do, set type for .rigth ('into var x') (unless is "null" or "undefined", they destroy type info)
             ...
             var leftType = this.left.getResultType();
             //if leftType instance of NameDeclaration and leftType.name not in ["undefined","null"]
             if (leftType instanceof NameDeclaration && ["undefined", "null"].indexOf(leftType.name)===-1) {
...
//        declare valid me.right.getResultType
       ...
       if (this.name === 'new') {
           ...
           return this.right.getResultType();// #me.right is Grammar.Operand
       };
    };
...
//declare var:type. alter on the fly scope var "types"
     ...
     if (this.specifier === 'types') {
         ...
         for ( var ac__inx=0,ac=undefined; ac__inx<this.varRef.accessors.length; ac__inx++) {
