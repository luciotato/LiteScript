//Compiled by LiteScript compiler v0.6.7, source: /home/ltato/LiteScript/devel/source/v0.7/Lexer.lite.md
// The Lexer
// =========
...
// Now we should escape internal d-quotes, but only *outside* string interpolation expressions
         ...
         var parsed = String.splitExpressions(line, this.stringInterpolationChar);
...
// Comments can also be multiline, starting with starting with ` `//` and ending with `
   ...
   var tokenPatterns = [['COMMENT', /^#(.*)$|^\/\/(.*)$/], ['NUMBER', /^0x[a-f0-9]+/i], ['NUMBER', /^[0-9]+(\.[0-9]+)?(e[+-]?[0-9]+)?/i], ['REGEX', /^(\/(?![\s=])[^[\/\n\\]*(?:(?:\\[\s\S]|\[[^\]\n\\]*(?:\\[\s\S][^\]\n\\]*)*])[^[\/\n\\]*)*\/)([imgy]{0,4})(?!\w)/], ['STRING', /^'(?:[^'\\]|\\.)*'/], ['STRING', /^"(?:[^"\\]|\\.)*"/], ['PUNC', /^\.\.\./], ['SPACE_DOT', /^\s+\./], ['SPACE_BRACKET', /^\s+\[/], ['WHITESPACE', /^[\f\r\t\v\u00A0\u2028\u2029 ]+/], ['ASSIGN', /^=/], ['ASSIGN', /^[\+\-\*\/\&]=/], ['LITERAL', /^(\+\+|--|->)/], ['LITERAL', /^[\(\)\[\]\;\,\.\{\}]/], ['OPER', /^(is|isnt|not|and|but|into|like|or|in|into|instance|instanceof|has|hasnt|bitand|bitor|bitxor|bitnot)\b/], ['OPER', /^(\*|\/|\%|\+|-|<>|>=|<=|>>|<<|>|<|!==|&)/], ['OPER', /^[\?\:]/], ['IDENTIFIER', /^[$A-Za-z_\x7f-\uffff][$\w\x7f-\uffff]*/]];
...
// with a call to core function "concat"
                ...
                // #else //-> JavaScript
                   ...
                   if (parsed.length && parsed[0][0] !== match[0]) { //match[0] is the quote: ' or "
                       parsed.unshift("''"); // prepend ''
                   };
