//Compiled by LiteScript compiler v0.6.7, source: /home/ltato/LiteScript/devel/source/v0.7/ASTBase.lite.md
...
// and it shows error message *and stack trace*.
    ...
    ASTBase.prototype.warn = function(msg){
       ...
       log.warning(this.positionText(), msg);
    };
...
// For strings we check the token **value** or **TYPE** (if searched is all-uppercase)
         ...
         if (typeof searched === 'string') {
           ...
           var isTYPE = searched[0] >= "A" && searched[0] <= "Z" && searched === searched.toUpperCase();
           var found = undefined;
...
// else, If `opt` returned nothing, we give the user a useful error.
       ...
       // if no result
       if (!result) {
         this.throwParseFailed("" + this.constructor.name + ":" + (this.extraInfo || '') + " found " + (this.lexer.token.toString()) + " but " + (this.listArgs(arguments)) + " required");
       };
