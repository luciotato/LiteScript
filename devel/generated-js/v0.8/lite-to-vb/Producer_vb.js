...
//Produce all vars & functions definitions first.
        ...
        this.out({COMMENT: "-----------"}, NL, "Module " + this.fileInfo.base, NL, {COMMENT: "-----------"}, NL);
    ...
    //append to class Grammar.Oper
      ...
      Grammar.Oper.prototype.declareIntoVar = function(){
          ...
          if (this.intoVar) {
              ...
              this.out("Dim ", this.right, " as Object", NL);
          };
      };
    ...
    //    append to class Grammar.ThrowStatement ###
      ...
      Grammar.ThrowStatement.prototype.produce = function(){
          ...
          if (this.specifier === 'fail') {
            ...
            this.out("Throw New System.Exception(", this.expr, ")");
          }
          //if .specifier is 'fail'
...
//or the only Operand of a unary oper.
    ...
    //    append to class Grammar.StringLiteral ###
      ...
      //method produce()
      // ---------------------------
      Grammar.StringLiteral.prototype.produce = function(){
          //.out '"', .getValue(), '"'
          this.out('"', this.getValue(), '"');
      };
...
//-(prettier generated code) do not add () for simple "falsey" variable check
        ...
        else if ((translated.charAt(0) >= 'a' && translated.charAt(0) <= 'z') || (translated.charAt(0) >= 'A' && translated.charAt(0) <= 'Z')) {
...
//example (`arguments` pseudo-array): `'lite' not in arguments` -> `Array.prototype.slice.call(arguments).indexOf(char)==-1`
        ...
        if (this.name === 'in') {
            ...
            this.out(prepend, this.right, ".Contains(", this.left, ")", append);
        }
        //if .name is 'in'
            ...
            this.out(prepend, 'TypeOf ', this.left, " is ", this.right, append);
        }
        //else if .name is 'instance of'
            ...
            this.out(prepend, this.left, ".Contains(", this.right, ")", append);
        }
        //else if .name is 'has property'
            ...
            this.out("Assign(", this.right, ",", this.left, ")");
        }
...
//the internal module var "arr[]", so the module will stop working as intended.
        ...
        var useme = this.name === 'this' ? 'Me' : this.name;
        //.out .preIncDec, preIfExported, useme, .accessors, .postIncDec
        this.out(this.preIncDec, preIfExported, useme, this.accessors, this.postIncDec);
      };
...
//Example: `default option = 5`
      ...
      Grammar.DefaultAssignment.prototype.produce = function(){
        ...
        this.out("if ismissing(", this.assignment.lvalue, ') then ', this.assignment.lvalue, "=", this.assignment.rvalue, ": end if", NL);
...
//default "produce" for VarDeclList is to out only names, comma separated
    ...
    //    append to class Grammar.TypeDeclaration ###
      ...
      //method produce()
      // ---------------------------
      Grammar.TypeDeclaration.prototype.produce = function(){
        //if .mainType.name is 'Map'
        if (this.mainType.name === 'Map') {
        
          //.out "new Dictionary(Of #{.keyType or 'Object'}, #{.itemType or 'Object'})"
          this.out("new Dictionary(Of " + (this.keyType || 'Object') + ", " + (this.itemType || 'Object') + ")");
        }
        //if .mainType.name is 'Map'
        
        else {
          //.out .mainType or 'Object'
          this.out(this.mainType || 'Object');
          //if .mainType, .out {COMMENT:"mainType: #{.mainType}  "}
          if (this.mainType) {this.out({COMMENT: "mainType: " + this.mainType + "  "})};
          //if .keyType, .out {COMMENT:"keyType: #{.keyType}  "}
          if (this.keyType) {this.out({COMMENT: "keyType: " + this.keyType + "  "})};
          //if .itemType, .out {COMMENT:"itemType: #{.itemType}"}
          if (this.itemType) {this.out({COMMENT: "itemType: " + this.itemType})};
        };
...
//'var' followed by a list of comma separated: var names and optional assignment
      ...
      Grammar.PropertiesDeclaration.prototype.produce = function(){
        ...
        for( var varDecl__inx=0,varDecl ; varDecl__inx<this.list.length ; varDecl__inx++){varDecl=this.list[varDecl__inx];
          ...
          this.out('    Public Property ', varDecl.name, " as ", varDecl.type || 'Object');
          //if varDecl.assignedValue, .out "=",varDecl.assignedValue
          if (varDecl.assignedValue) {this.out("=", varDecl.assignedValue)};
          //.out NL
          this.out(NL);
      ...
      // ---------------------------
      Grammar.PropertiesDeclaration.prototype.produceAssignment = function(prefix){
...
//in LiteScript, 'var' declaration assigns 'undefined')
          ...
          if (this.parent instanceof Grammar.VarStatement) {
              ...
              this.out(' = ', this.assignedValue || 'Nothing');
          }
          //if .parent instanceof Grammar.VarStatement
    ...
    //    append to class Grammar.IfStatement ###
      ...
      Grammar.IfStatement.prototype.produce = function(){
        ...
        if (this.body instanceof Grammar.SingleLineBody) {
            ...
            this.out("if ", this.conditional, ' Then ', this.body);
        }
        //if .body instanceof Grammar.SingleLineBody
        
        else {
            //.out "if ", .conditional, " Then", NL // .getEOLComment()
            this.out("if ", this.conditional, " Then", NL);
            //.out  .body, NL
    ...
    //    append to class Grammar.ElseStatement ###
      ...
      Grammar.ElseStatement.prototype.produce = function(){
        ...
        this.out(NL, "else", NL, this.body, NL);
      };
...
//Create a default index var name if none was provided
        ...
        if (this.keyIndexVar) {
            ...
            this.out("Dim ", indexVarName, " as Integer =", startValue, NL);
        };

        //.out "For Each ", .valueVar.name, " in ", iterable,NL
        this.out("For Each ", this.valueVar.name, " in ", iterable, NL);
        //.out .where
        this.out(this.where);
        //.out .body
        this.out(this.body);
        //if .where, .out "End if",NL
        if (this.where) {this.out("End if", NL)};

        //.out "Next",{COMMENT:[" each in ",.iterable]},NL
        this.out("Next", {COMMENT: [" each in ", this.iterable]}, NL);
      };
...
//When Map is implemented using js "Object"
      ...
      Grammar.ForEachInArray.prototype.produceInMap = function(iterable){
          ...
          this.out("for Each item in ", iterable, NL);
          //.body.out 'Dim ',indexVarName, " as string = item.key",NL
          this.body.out('Dim ', indexVarName, " as string = item.key", NL);
          //if .valueVar
          if (this.valueVar) {
          
              //.body.out 'Dim ',.valueVar.name,"=item.value",NL
              this.body.out('Dim ', this.valueVar.name, "=item.value", NL);
          };
          //.out .where
          this.out(this.where);
          //.out .body
          ...
          this.out("Next", {COMMENT: " for each item in map"}, NL);
      };
...
//Examples: `for n=0 while n<10`, `for n=0 to 9`
      ...
      Grammar.ForIndexNumeric.prototype.produce = function(){
        ...
        if (isToDownTo) {
            ...
            this.out("for ", this.keyIndexVar.name, " As Integer =", this.keyIndexVar.assignedValue || "0");
            //.out .keyIndexVar.name, .conditionPrefix is 'to'? " to " else " downto ", .endExpression
            this.out(this.keyIndexVar.name, this.conditionPrefix === 'to' ? " to " : " downto ", this.endExpression);
            //.out .body,NL
            this.out(this.body, NL);
            //.out "Next", NL
            this.out("Next", NL);
...
//Produce the condition, negated if the prefix is 'until'.
            ...
            this.out("Dim ", this.keyIndexVar.name, " As Integer =", this.keyIndexVar.assignedValue || "0", NL);
            //.out "Do While ", .endExpression.produce( negated = .conditionPrefix is 'until' ),NL
            this.out("Do While ", this.endExpression.produce(this.conditionPrefix === 'until'), NL);
            //.out .body,NL
            this.out(this.body, NL);
            //.out .keyIndexVar.name,"+=1", NL
            this.out(this.keyIndexVar.name, "+=1", NL);
            //.out "Loop", NL
            this.out("Loop", NL);
        };
      };
...
//`ForWhereFilter: [where Expression]`
      ...
      Grammar.ForWhereFilter.prototype.produce = function(){
        ...
        this.out('if ', this.filterExpression, ' Then', NL);
      };
...
//(one NameValuePair per line, indented, comma is optional)
     ...
     Grammar.ObjectLiteral.prototype.produce = function(){
          ...
          if (this.isMap) {this.out('new Dictionary(Of String,Object) from ')};
...
//`generator` prefix marks a 'generator' function that can be paused by `yield` (js/ES6 function*)
     ...
     Grammar.FunctionDeclaration.prototype.produce = function(prefix){
      ...
      if (this instanceof Grammar.MethodDeclaration) {
          ...
          this.out("Public Function ", this.name, " ");
      }
      //if this instance of Grammar.MethodDeclaration
...
//if the function has a exception block, insert 'try{'
      ...
      if (!this.body || !this.body.statements) {
            ...
            this.sayErr('function ' + this.name + ' from ' + this.lexer.filename + ' has no body');
            //.out "**NO BODY**",NL
            this.out("**NO BODY**", NL);
            //.out "end function",NL
            this.out("end function", NL);
            //return
            return;
      };
...
//`print` is an alias for console.log
      ...
      Grammar.PrintStatement.prototype.produce = function(){
        ...
        this.out("Debug.Print(", {"CSL": this.args}, ")");
      };
...
//we need to get the class constructor, and separate other class items.
        ...
        //.out "Class ",.name,NL
        this.out("Class ", this.name, NL);
        //if .varRefSuper
        if (this.varRefSuper) {
        
          //.out "Inherits ",.varRefSuper,NL
          this.out("Inherits ", this.varRefSuper, NL);
        };
...
//constructor
        ...
        if (this.constructorDeclaration) {
            ...
            this.out("Sub New(", this.constructorDeclaration.paramsDeclarations, ")", NL);
        }
        //if .constructorDeclaration //there was a constructor body, add specified params
        
        else {
            //.out "Sub New() 'default constructor",NL
            this.out("Sub New() 'default constructor", NL);
        };
...
//call super-class __init
        ...
        if (this.varRefSuper) {
              ...
              this.out({COMMENT: ["default constructor: call super.constructor"]}, NL, "    MyBase.New(");
              //if .constructorDeclaration and .constructorDeclaration.paramsDeclarations
              if (this.constructorDeclaration && this.constructorDeclaration.paramsDeclarations) {
              
                 //var names=[]
                 var names = [];
                 //for each varDecl in .constructorDeclaration.paramsDeclarations.list
                 for( var varDecl__inx=0,varDecl ; varDecl__inx<this.constructorDeclaration.paramsDeclarations.list.length ; varDecl__inx++){varDecl=this.constructorDeclaration.paramsDeclarations.list[varDecl__inx];
                 
                    //names.push varDecl.name
                    names.push(varDecl.name);
                 };// end for each in this.constructorDeclaration.paramsDeclarations.list
                 //.out {CSL:names}
                 this.out({CSL: names});
                 //.out ")",NL
                 this.out(")", NL);
...
//If the class was adjectivated 'export', add to module.exports
        ...
        this.out(NL, "end class '", this.name, NL);
      };
...
//Append-to body contains properties and methods definitions.
      ...
      Grammar.AppendToDeclaration.prototype.produce = function(){
        ...
        this.out("Partial Class ", this.varRef, NL);
        //.out .body
        this.out(this.body);
        //.out NL
        this.out(NL);
        //.out "End Class 'partial"
        this.out("End Class 'partial");
        //.skipSemiColon = true
        this.skipSemiColon = true;
      };
...
//Append-to body contains properties and methods definitions.
      ...
      Grammar.NamespaceDeclaration.prototype.produce = function(){
        ...
        this.out('Namespace ', this.name, NL);
        //.out .body
        this.out(this.body);
      };
    ...
    //    append to class Grammar.TryCatch ###
      ...
      Grammar.TryCatch.prototype.produce = function(){
        ...
        this.out("try ", NL, this.body, this.exceptionBlock, NL);
        //.out "End try",NL
        this.out("End try", NL);
      };
    ...
    //    append to class Grammar.ExceptionBlock ###
      ...
      Grammar.ExceptionBlock.prototype.produce = function(){
        ...
        this.out(NL, "Catch ", this.catchVar);
        //.out .body
        this.out(this.body);
...
//if we have a varRef, is a case over a value
        ...
        this.out('select ', this.varRef, NL);
        ...
        for( var index=0,whenSection ; index<this.cases.length ; index++){whenSection=this.cases[index];
            ...
            this.out('case ', {CSL: whenSection.expressions, separator: ',', freeForm: 0}, NL);
            //.out whenSection.body, NL,NL
            this.out(whenSection.body, NL, NL);
...
//else body
        ...
        if (this.elseBody) {
            ...
            this.out(NL, 'case else', NL, this.elseBody, NL);
        };
      };
...
//Helper methods and properties, valid for all nodes
     ...
     ASTBase.prototype.assignIfUndefined = function(name, value){
          ...
          this.out("if isMissing(", name, ') then ', name, "=", value, ": end if", NL);
     };
...
// -----------
// end of module
//# sourceMappingURL=Producer_vb.js.map
