...
//Module vars:
   ...
   var nameAffinity = new NameDeclaration('Name Affinity');//# project-wide name affinity for classes
...
//it also follows the __proto__ chain (same mechanism as js __proto__ chain)
...
//We use a member named '__proto__' on NameDeclarations, mapping run-time proto chain.
//We follow the chain to validate property names.

           var nextInChain = actual.findOwnMember('__proto__');
...
//returns '__proto__' converted type

       //me.convertType '__return type__'  #a Function can have __return type__
       this.convertType('__return type__');//#a Function can have __return type__
       //me.convertType '__item type__'  #an Array can have __item type__ e.g.: 'var list: string array'
       this.convertType('__item type__');//#an Array can have __item type__ e.g.: 'var list: string array'
...
//in scope, var 'token' is assumed type 'Token', return true if type was assigned
       ...
       //if not me.findOwnMember('__proto__')
       if (!(this.findOwnMember('__proto__'))) {
         converted = this.assignTypebyNameAffinity();
       }
       else {
         converted = this.convertType('__proto__');//#other objects
       ...
       if (this.nodeDeclared && !(String.isCapitalized(this.name))) {
           ...
           if (!(this.findOwnMember('__proto__'))) {
               ...
               if (possibleClassRef && possibleClassRef.nodeDeclared.nameDecl.members.prototype) {
                   //me.setMember('__proto__', possibleClassRef.nodeDeclared.nameDecl.members.prototype)
                   this.setMember('__proto__', possibleClassRef.nodeDeclared.nameDecl.members.prototype);
                   //return true
                   return true;
               };
           };
...
//Add a built-in class to global scope, return class prototype
     ...
     //if nameDecl.members.prototype
     if (nameDecl.members.prototype) {
       //nameDecl.setMember '__return type__', nameDecl.members.prototype
       nameDecl.setMember('__return type__', nameDecl.members.prototype);
       //return nameDecl.members.prototype
       return nameDecl.members.prototype;
     };
...
//Initialize the global scope
        ...
        //declare valid project.globalScope
        //declare valid project.root.compilerVars.addMember
        //declare valid project.root.compilerVars.addMember
...
//compiler vars, to use at conditional compilation
       ...
       project.root.compilerVars = new NameDeclaration("Compiler Vars");
       //project.root.compilerVars.addMember new NameDeclaration('debug',{value:true})
       project.root.compilerVars.addMember(new NameDeclaration('debug', {value: true}));
        //log.message project.root.compilerVars
        //log.message ""


        //if me.options.addGlobals, globalScope.addMembers me.options.addGlobals
         ...
         for ( var node__inx=0; node__inx<moduleNode.requireCallNodes.length; node__inx++) {
           ...
           if (node.importedModule) {
             ...
             var reference = undefined;
              ...
              //#get parent of the "require" call
              //parent = node.parent
             parent = node.parent;
             ...
             if (parent instanceof Grammar.Operand) {
              ...
              //#get referece where import module is assigned to
...
//Inform unconverted types as errors
       ...
       if (totalConverted < NameDeclaration.allOfThem.length) {
         ...
         for ( var nameDecl__inx=0; nameDecl__inx<NameDeclaration.allOfThem.length; nameDecl__inx++) {
           ...
           var type = nameDecl.findOwnMember('__proto__');
...
//Adds a member to a NameDecl, referencing this node as nodeDeclared
    ...
    //     helper method tryGetMember(nameDecl,name,options)
...
//If the found item has a different case than the name we're looking for, emit error
           ...
           else if (options.informError) {
               //me.sayErr "UNDECLARED NAME: '#{name}'"
               this.sayErr("UNDECLARED NAME: '" + (name) + "'");
               found = this.addToScope(name, {undeclared: true});
               //if String.isCapitalized(name) #let's assume is a class
               if (String.isCapitalized(name)) {//#let's assume is a class
                 //me.addMemberTo(found,'prototype',{undeclared:true})
                 this.addMemberTo(found, 'prototype', {undeclared: true});
               };
           };
...
//to get their 'owner', i.e., a NameDeclaration where they'll be added as members
       ...
       var ownerDecl = undefined, optClass = true;
        ...
        //#in OLD SYNTAX - "method XX OF x.y.z", me.varRefOwner is preset
        //declare valid me.varRefOwner.tryGetReference
        //declare valid me.specifier
        //declare valid me.specifier

        //if no me.varRefOwner
       //if no me.varRefOwner
       if (!this.varRefOwner) {
           ...
           if (parent instanceof Grammar.AppendToDeclaration) {
             ...
             if (!parent.nameDecl) {
              ...
              //if no parent.nameDecl.members.prototype, me.sayErr "Unexpected. Class has no prototype"
             //if no parent.nameDecl.members.prototype, me.sayErr "Unexpected. Class has no prototype"
             if (!parent.nameDecl.members.prototype) {
                 this.sayErr("Unexpected. Class has no prototype")};
             //return parent.nameDecl.members.prototype
             return parent.nameDecl.members.prototype;
       ...
       //if no ownerDecl
       if (!ownerDecl) {
         //if options.informError, me.sayErr "Append to/method of: '#{me.varRefOwner}'. Reference is not fully declared"
         if (options.informError) {
             this.sayErr("Append to/method of: '" + (this.varRefOwner) + "'. Reference is not fully declared")};
         //return
         return;
       };
       ...
       if (optClass) {
         ...
         //if no ownerDecl
         if (!ownerDecl) {
           //if options.informError, me.sayErr "Append to class/method of: '#{me.varRefOwner}'. Reference has no .prototype"
           if (options.informError) {
               this.sayErr("Append to class/method of: '" + (this.varRefOwner) + "'. Reference has no .prototype")};
           //return
           return;
         };
       };
  ...
  //`function x ( a : string = 'some text', b, c=0)`
     ...
     Grammar.VariableDecl.prototype.getTypeFromAssignedValue = function(){
       ...
       if (this.nameDecl && this.assignedValue) {
           var type = this.nameDecl.findOwnMember('__proto__');
           //if no type
           if (!type) {
               var result = this.assignedValue.getResultType();
               //if result, me.nameDecl.setMember('__proto__', result)
               if (result) {
                   this.nameDecl.setMember('__proto__', result)};
           };
       };
     };
...
//'prototype' starts with 'constructor' which is a pointer to the class-funcion itself
       ...
       //if me.varRefSuper, prtypeNameDecl.setMember('__proto__',me.varRefSuper)
       if (this.varRefSuper) {
           prtypeNameDecl.setMember('__proto__', this.varRefSuper)};
...
//returnType of the class-function, is the prototype
       ...
       //me.nameDecl.setMember('__return type__',prtypeNameDecl)
       this.nameDecl.setMember('__return type__', prtypeNameDecl);
...
//Now create function's scope, using 'owner' as var this's __proto__
...
//Now that we have 'owner' we can set __proto__ for scope var 'this',
//so we can later validate `.x` in `this.x = 7`
     ...
     if (owner) {
         ...
         this.addMethodToOwner(owner);
          ...
          //me.scope.members.this.setMember '__proto__',owner
         //me.scope.members.this.setMember '__proto__',owner
         this.scope.members.this.setMember('__proto__', owner);
     };
    };


    //     helper method addMethodToOwner(owner)
    Grammar.FunctionDeclaration.prototype.addMethodToOwner = function(owner){
     ...
     //if me.returnType, me.nameDecl.setMember '__return type__', me.returnType
     if (this.returnType) {
         this.nameDecl.setMember('__return type__', this.returnType)};
     this.declared = true;
    };
...
//If no mainVar.type, guess type from iterable's itemType
       ...
       if (this.variant instanceof Grammar.ForEachInArray) {
           //if no me.mainNameDecl.findOwnMember('__proto__')
           if (!this.mainNameDecl.findOwnMember('__proto__')) {
               var iterableType = this.variant.iterable.getResultType();
               //if iterableType
               if (iterableType) {
                 var itemType = iterableType.findOwnMember('__item type__');
                 //if itemType
                 if (itemType) {
                   //me.mainNameDecl.setMember('__proto__',itemType)
                   this.mainNameDecl.setMember('__proto__', itemType);
                 };
               };
           };
       }
       else if (this.variant instanceof Grammar.ForEachProperty) {
           //me.indexNameDecl.setMember('__proto__',globalPrototype('String'))
           this.indexNameDecl.setMember('__proto__', globalPrototype('String'));
       };
    };
...
//for PropertyAccess
           ...
           if (ac instanceof Grammar.PropertyAccess) {
               ...
               actualVar = this.tryGetMember(actualVar, '__item type__');
           }
           else if (ac instanceof Grammar.FunctionAccess) {
               actualVar = this.tryGetMember(actualVar, '__return type__');
           };
...
//check if we've got a a clear reference.
     ...
     if (!(reference instanceof NameDeclaration)) {
         return};
     //if reference.findOwnMember('__proto__'), return #has a type already
     if (reference.findOwnMember('__proto__')) {
         return};
...
//if we do, make lvalue point to rvalue
     ...
     //if rvalueType instance of NameDeclaration, reference.makePointTo rvalueType
     if (rvalueType instanceof NameDeclaration) {
         reference.makePointTo(rvalueType)};
    };
...
//alter on the fly scope var "types"
     ...
     if (this.specifier === 'types') {
         ...
         for ( var varDecl__inx=0; varDecl__inx<this.names.length; varDecl__inx++) {
             ...
             //if mainVar, mainVar.setMember '__proto__', varDecl.nameDecl.ownMember('__proto__')
             if (mainVar) {
                 mainVar.setMember('__proto__', varDecl.nameDecl.ownMember('__proto__'))};
         }; // end for each in this.names
     }
