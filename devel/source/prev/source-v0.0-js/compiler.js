(function () {
  ...
  /* The LiteScript Compiler
     ----------------

     LiteScript is a highly readable language that compiles to JavaScript.

     The LiteScript compiler is written in LiteScript. As a result, a "preiously compiled" (JavaScript)
     version of the compiler is required to compile this source. You can obtain the latest precompiled package from
     npm using `npm install -g litescript` (may require `sudo` depending on your setup).
     Once you have LiteScript installed globally, you can use the following scripts:

     * `npm run make` - This will compile the `sources` directory (this source) into the `compiled` directory.
     ...
     * `npm run bootstrap` - Build `sources` using the globally installed LiteScript compiler,
     then rebuild `sources` using the compiled version of itself. This also runs tests.
     This script is used to verify the compiler before deployment to npm.

     Structure
     =========

     The compiler uses several stages to compile LiteScript code.

     For Literate LiteScript files (like this one), the `literate` module strips out the leading spaces from code
     blocks and turns Markdown syntax (like this line) into comments.
      */
  literate = require('./literate');
  /*
     The first stage is the lexer, which turns the raw string output into an array of tokens of various types
     (such as `IDENTIFIER`, `STRING`, and `NUMBER`).
      */
  lexer = require('./lexer');
  /*
     The "sugar" stage handles syntactic sugar. This includes features that would be difficult to handle
     in the full parsing stage, such as function calls without parentheses, multiline lists, and CoffeeScript style function definitions.
     The result is a modified array of tokens that can be read by the parser.
      */
  sugar = require('./sugar');
  /*
  ...
  /*
     ...
     The compile function takes a `code` parameter that can be a string or buffer. This is the raw LiteScript (or Literate LiteScript) code. It also takes an `options` object which _may_ contain the following properties (all optional):
     ...
     * `literate` - compile as a Literate LiteScript file. The default is false (regular LiteScript).
  ...
  function compile(code, options) {
    ...
    try {
      ...
      console.log('COMPILE ERROR: ', compile_error.message);
      console.log('full: ', JSON.stringify(compile_error));
      if (typeof compile_error === 'object') console.log(compile_error.stack)
      //console.trace('quitting kal');
      process.exit(1)  //process.exit is not defined on the browser 
      //throw compile_error;
    }
  }
  ...
  /*
     ...
     This segment adds extensions to node's `require` function for LiteScript and Literate LiteScript files so that you can just `require` a LiteScript file without having to compile it first (assuming your script has already run `require 'kal'`).
      */
  if (require.extensions) {
    extension = function (module, filename) {
      var is_literate, content;
      /*
         Check if this is a Literate LiteScript file.
          */
