...
#include "exceptions.h"
   ...
   //import
   #include "ASTBase"
   var ASTBase = _Module_ASTBase_exports;
...
//## Main API functions: LiteScript.compile & LiteScript.compileProject
   ...
   extern var compile(var this, var arguments){
       enum arguments_enum={filename, sourceLines, options};
...
//* string, compiled code
       ...
       if (_getProp(arguments[options],"storeMessages") {
           _o(_o(log)->v[_log_options])->v[_log_options_storeMessages] = true;
           //log.getMessages()
           _call(log,_log_getMessages,0); //clear
           _o(log)->v[_log_getMessages].val.functionPtr(log,0);
       };

       //var moduleNode = compileModule(filename, sourceLines, options)
       var moduleNode = _call(this,_Compiler_compileModule,_args(3,arguments[filename], arguments[sourceLines], arguments[options]));
       var moduleNode = _o(this)->v[_Compiler_compileModule].val.functionPtr(this,_args(3,arguments[filename], arguments[sourceLines], arguments[options]));
       #define CALL(THIS, INX, ARGS){_o(THIS)->v[INX].val.functionPtr(THIS,ARGS);}
       var moduleNode = CALL(this,_Compiler_compileModule,_args(3,arguments[filename], arguments[sourceLines], arguments[options]));

       //return moduleNode.getCompiledText();
       return CALL(moduleNode,_Grammar_Module_getCompiledText,0);
   };


   //export function compileProject (mainModule, options) returns Project
   extern var compileProject(var this, var arguments){
       enum arguments_enum={mainModule, options};
...
//another modules via import|require.
       ...
       //default options =
       enum _LOCAL_options_enum={_LOCAL_options_outDir,_LOCAL_options_target};
       _defaultObject(&arguments[options])
       _default(arguments[options],"outDir",_s("."));
       _default(arguments[options],"target",_s("js"));

       //log.extra "Out Dir: #{options.outDir}"
       CALL(log,_log_extra,_args(1,_sinterp("Out Dir: %s",_getProp(arguments[options],"outDir"));

//Create a 'Project' to hold the main module and dependant modules

       //var project = new Project(mainModule, options);
       var project = Object_Create(Project,_args(2,arguments[mainModule], arguments[options]);

       //project.compile();
       CALL(project,_Project_compile,_args(0));
