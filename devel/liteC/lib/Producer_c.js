...
// Method get a trailing "_" if they're a C reserved word
   ...
   var coreSupportedMethods = ["toString", "tryGetMethod", "tryGetProperty", "getProperty", "getPropertyName", "has", "get", "set", "clear", "delete", "keys", "slice", "split", "indexOf", "lastIndexOf", "concat", "toUpperCase", "toLowerCase", "charAt", "replaceAll", "trim", "toDateString", "toTimeString", "toUTCString", "toISOString", "shift", "push", "unshift", "pop", "join", "splice"];

   var coreSupportedProps = ['name', 'value', 'message', 'stack', 'code'];
...
// create _dispatcher.c & .h
       ...
       module.exports.dispatcherModule = new Grammar.Module(project);
   ...
   function normalizeDefine(name){
       ...
       var _end4=name.length;
       for( var n=0; n<=_end4; n++) {
...
// header
       ...
       this.out("\n\n//--------------", NL, new Map().fromObject({COMMENT: c}), this.varRefSuper ? [' extends ', this.varRefSuper, NL] : '', NL, NL, "extern any ", c, "; //Class Object", NL, NL);
...
// In C we create a struct for "instance properties" of each class
       ...
       for( var methodName__inx=0,methodName ; methodName__inx<classMethods.length ; methodName__inx++){methodName=classMethods[methodName__inx];
           this.out("extern any ", c, "_", methodName, "(DEFAULT_ARGUMENTS);", NL);
...
// this is the class body, goes on the .c file,
       ...
       this.out(new Map().fromObject({COMMENT: c}), NL, NL, 'any ' + c + '; //Class Object', NL);
...
// static definition info for each class: list of _METHODS and _PROPS
    ...
    Grammar.ClassDeclaration.prototype.produceClassRegistration = function(){
       ...
       // if .varRefSuper
       if (this.varRefSuper) {
           superClass = this.varRefSuper.tryGetReference();
           // if no superClass, .throwError "cannot get a reference to #{.varRefSuper}"
           if (!superClass) {this.throwError("cannot get a reference to " + this.varRefSuper)};
...
// recurse for namespaces
                ...
                // declare item.specific:Grammar.NamespaceDeclaration
                // #call namespaceInit
               this.out('    ', item.specific.makeName(), '__namespaceInit();', NL);
           };
       };// end for each in this.statements
...
// To ease reading of compiled code, add original Lite line as comment
       ...
       if (this.lexer.options.comments) {
         ...
         if (this.lexer.outCode.lastOriginalCodeComment < this.lineInx) {
           ...
           if (!(([Grammar.CompilerStatement, Grammar.DeclareStatement, Grammar.DoNothingStatement].indexOf(this.specific.constructor)>=0))) {
             this.out(new Map().fromObject({COMMENT: this.lexer.infoLines[this.lineInx].text.trim()}), NL);
           };
         };
         this.lexer.outCode.lastOriginalCodeComment = this.lineInx;
       };
...
// declare vars before the statement (exclude body of FunctionDeclaration)
       ...
       this.callOnSubTree(methodToCall, Grammar.Body);
...
// else, for FunctionAccess
               ...
               if (callNew) {
                   ...
                   if (!hasInstanceReference) { //first accessor is function access, this is a call to a global function
                       ...
                       if (functionAccess.args && functionAccess.args.length <= 4) {
   ...
   // append to class Names.Declaration ###
     ...
     Names.Declaration.prototype.addToAllProperties = function(){
       ...
       if (coreSupportedProps.indexOf(name)===-1 && !(allPropertyNames.has(name))) {
           ...
           if (allMethodNames.has(name)) {
               this.sayErr("A method named '" + name + "' is already defined. Cannot reuse the symbol for a property");
...
// create a var holding object property count
       ...
       this.out("len_t ", uniqueName, "_len=", listName, '.class->instanceSize / sizeof(any);', NL);
       ...
       this.out("for(int ", intIndexVarName, "=", startValue, " ; ", intIndexVarName, "<", uniqueName, "_len", " ; ", intIndexVarName, "++){");

       this.body.out(this.mainVar.name, "=", listName, ".value.prop[", intIndexVarName, "];", NL);

       // if .indexVar
       if (this.indexVar) {
           this.body.out(this.indexVar.name, "= _getPropertyName(", listName, ",", intIndexVarName, ");", NL);
       };
...
// include mainVar.name in a bracket block to contain scope
     ...
     Grammar.ForEachInArray.prototype.produceForMap = function(listName){
       ...
       var intIndexVarName = "" + this.mainVar.name + "__inx";// # unique map numeric index
       var nvp = UniqueID.getVarName('nvp');// # pointer to name-value pair[inx]

       this.out("{ NameValuePair_ptr ", nvp, "=NULL; //name:value pair", NL);
       // if .indexVar, .body.out " var ",.indexVar.name,"=undefined; //key",NL
       if (this.indexVar) {this.body.out(" var ", this.indexVar.name, "=undefined; //key", NL)};
       ...
       if (this.indexVar) {this.body.out(this.indexVar.name, "=", nvp, "->name;", NL)};
       this.body.out(this.mainVar.name, "=", nvp, "->value;", NL); //get value
...
// Produce a Method
     ...
     Grammar.MethodDeclaration.prototype.produce = function(){
       ...
       // if isClass
       if (isClass) {
           this.body.out("assert(_instanceof(this,", c, "));", NL, "//---------", NL);
       };
...
// Except is the same name as the top namespace|class (auto export default).
       ...
       // method isClass
       Names.Declaration.prototype.isClass = function(){
           return !(this.isNamespace) && this.findOwnMember('prototype');
       };

       // method isMethod
       Names.Declaration.prototype.isMethod = function(){
           // if .isNamespace or .isClass(), return false
           if (this.isNamespace || this.isClass()) {return false};
           var prt = this.findOwnMember('**proto**');
           return prt && prt.parent && prt.parent.name === 'Function';
       };

       // method isProperty
       Names.Declaration.prototype.isProperty = function(){
           return !(this.isNamespace) && !(this.isClass()) && !(this.isMethod());
       };
...
// For C production, we're declaring each distinct method name (verbs)
       ...
       Names.Declaration.prototype.addToAllMethodNames = function(){
           ...
           if (coreSupportedMethods.indexOf(methodName)===-1 && !(allMethodNames.has(methodName))) {
               ...
               if (allPropertyNames.has(methodName)) {
                   this.sayErr("A property '" + methodName + "' is already defined. Cannot reuse the symbol for a method.");
                   allPropertyNames.get(methodName).sayErr("Definition of property '" + methodName + "'.");
               }
               
               else if (coreSupportedProps.indexOf(methodName)>=0) {
                   this.sayErr("A property '" + methodName + "' is defined in core. Cannot reuse the symbol for a method.");
               }
