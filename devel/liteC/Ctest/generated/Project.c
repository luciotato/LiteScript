...
// add 'inNode' and 'inBrowser' as compiler vars
        ...
        var targetDef = _concatAny(2,(any_arr){any_str("TARGET_"), __call(toUpperCase_,PROP(target_,options),0,NULL)});
        ...
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
          ...
          if (!(_anyToBool(PROP(isCore_,PROP(fileInfo_,moduleNode)))) && _anyToBool(PROP(referenceCount_,moduleNode)))  {
            ...
            if (!(_anyToBool(PROP(isLite_,PROP(fileInfo_,moduleNode)))))  {
                ...
                result = _concatAny(2,(any_arr){(_anyToBool(__or1=any_number(_length(contents) >> 10))? __or1 : any_number(1)), any_str(" kb")});
                //contents=undefined
                contents = undefined;
            }
            //else
...
// save to disk / add to external cache
                ...
                if (_length(resultLines))  {
                    ...
                    Environment_externalCacheSave(undefined,2,(any_arr){_concatAny(2,(any_arr){__call(slice_,PROP(outFilename_,PROP(fileInfo_,moduleNode)),2,(any_arr){any_number(0), any_number(-1)}), any_str("h")}), resultLines});
                };
                //end if
...
// Handle errors, add stage info, and stack
        ...
        //exception err
            ...
            //else
                ...
                PROP(message_,err) = _concatAny(3,(any_arr){__call(posToString_,PROP(lexer_,moduleNode),0,NULL), any_str("\n"), (_anyToBool(__or3=PROP(stack_,err))? __or3 : PROP(message_,err))});
            };
...
//#### Method produceModule(moduleNode:Grammar.Module)
     any Project_produceModule(DEFAULT_ARGUMENTS){
        ...
        if (_anyToNumber(PROP(comments_,PROP(options_,this))) >= 2)  {
            ...
            __call(put_,PROP(outCode_,PROP(lexer_,moduleNode)),1,(any_arr){_concatAny(4,(any_arr){any_str("//Compiled by LiteScript compiler v"), PROP(version_,PROP(options_,this)), any_str(", source: "), PROP(filename_,PROP(fileInfo_,moduleNode))})});
        ...
        if (!_anyToBool(PROP(nomap_,PROP(options_,this))) && __is(PROP(outExtension_,PROP(fileInfo_,moduleNode)),any_str("js")))  {
            ...
            __call(put_,PROP(outCode_,PROP(lexer_,moduleNode)),1,(any_arr){_concatAny(4,(any_arr){any_str("//# sourceMappingURL="), PROP(base_,PROP(fileInfo_,moduleNode)), PROP(outExtension_,PROP(fileInfo_,moduleNode)), any_str(".map")})});
        };
     return undefined;
     }
