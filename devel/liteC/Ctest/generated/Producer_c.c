...
, Producer_c_NL, _concatAny(3,any_str("#ifndef "), Producer_c_normalizeDefine(undefined,1,(any_arr){PROP(outRelFilename_,PROP(fileInfo_,this))}), any_str("_H")), Producer_c_NL, _concatAny(3,any_str("#define "), Producer_c_normalizeDefine(undefined,1,(any_arr){PROP(outRelFilename_,PROP(fileInfo_,this))}), any_str("_H")), Producer_c_NL, Producer_c_NL});
        ...
        var dispatcherFull = _concatAny(2,Environment_dirName(undefined,1,(any_arr){PROP(outFilename_,PROP(fileInfo_,PROP(parent_,this)))}), any_str("/_dispatcher.h"));
...
//(Modules act as namespaces, var=property, function=method)
        ...
        if (_anyToBool(PROP(isScope_,PROP(parent_,PROP(nameDecl_,this)))))  { //is a namespace declared at module scope
            ...
            //else
                ...
                prefix = _concatAny(3,PROP(base_,PROP(fileInfo_,moduleNode)), any_str("_"), prefix);
            };
            //end if
...
//methods in the class as namespace
        ...
        any _list68=PROP(members_,PROP(nameDecl_,this));
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        int64_t __len=MAPSIZE(_list68); //how many pairs
        var classMethodNameDecl=undefined; //value
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
                __nvp = MAPITEM( __inx,_list68);
                classMethodNameDecl= __nvp->value;
          
            //where classMethodNameDecl.name isnt 'prototype' and classMethodNameDecl.name.charAt(0) isnt '*'
            if(!__is(PROP(name_,classMethodNameDecl),any_str("prototype")) && !__is(__call(charAt_,PROP(name_,classMethodNameDecl),1,(any_arr){any_number(0)}),any_str("*"))){
                //if not classMethodNameDecl.isProperty
                if (!(_anyToBool(PROP(isProperty_,classMethodNameDecl))))  {
                    ...
                    METHOD(out_,this)(this,6,(any_arr){any_str("extern any "), c, any_str("_"), PROP(name_,classMethodNameDecl), any_str("(DEFAULT_ARGUMENTS); //class as namespace"), Producer_c_NL});
                };
        }}};// end for each in map PROP(members_,PROP(nameDecl_,this))
...
//this is the class body, goes on the .c file,
        ...
        { var item=undefined;
        for(int index=0 ; index<_list69.value.arr->length ; index++){item=ITEM(index,_list69);
...
, any_str(";"), Producer_c_NL});};
            ...
            else if (__is(_switch6,Grammar_FunctionDeclaration)||__is(_switch6,Grammar_MethodDeclaration)){
...
//User classes must be registered previously, in case the module vars use them as initial values.
        ...
        for(int item__inx=0 ; item__inx<_list77.value.arr->length ; item__inx++){item=ITEM(item__inx,_list77);
            ...
            if(_instanceof(PROP(specific_,item),Grammar_VarDeclList)){
                ...
                any _list78=PROP(list_,PROP(specific_,item));
                { var variableDecl=undefined;
                for(int variableDecl__inx=0 ; variableDecl__inx<_list78.value.arr->length ; variableDecl__inx++){variableDecl=ITEM(variableDecl__inx,_list78);
...
//if there is var or properties with assigned values, produce those assignment
        //for each item in .statements 
        any _list79=PROP(statements_,this);
        { var item=undefined;
        for(int item__inx=0 ; item__inx<_list79.value.arr->length ; item__inx++){item=ITEM(item__inx,_list79);
            ...
            if(__is(any_class(PROP(specific_,item).class),Grammar_PropertiesDeclaration)){
                ...
                //for each variableDecl in item.specific.list
                any _list80=PROP(list_,PROP(specific_,item));
                { var variableDecl=undefined;
                for(int variableDecl__inx=0 ; variableDecl__inx<_list80.value.arr->length ; variableDecl__inx++){variableDecl=ITEM(variableDecl__inx,_list80);
...
//add comment lines, in the same position as the source
        ...
        METHOD(outSourceLinesAsComment_,this)(this,2,(any_arr){any_number(0), any_number(_anyToNumber(PROP(sourceLineNum_,this)) - 1)});
...
//now follow each accessor
       ...
       // 
        ...
        any _list82=PROP(accessors_,this);
        { var ac=undefined;
        for(int inx=0 ; inx<_list82.value.arr->length ; inx++){ac=ITEM(inx,_list82);
...
//for PropertyAccess
            ...
            if (_instanceof(ac,Grammar_PropertyAccess))  {
                ...
                else if (_anyToBool(actualVar) && _anyToBool((_anyToBool(__or18=PROP(isNamespace_,actualVar))? __or18 : METHOD(findOwnMember_,actualVar)(actualVar,1,(any_arr){any_str("prototype")}))))  {
...
//### Append to class Grammar.VarDeclList ###
      ...
      any Grammar_VarDeclList_getNames(DEFAULT_ARGUMENTS){
        ...
        //for each varDecl in .list
        any _list85=PROP(list_,this);
        { var varDecl=undefined;
        for(int varDecl__inx=0 ; varDecl__inx<_list85.value.arr->length ; varDecl__inx++){varDecl=ITEM(varDecl__inx,_list85);
...
//### Append to class Grammar.SingleLineBody ###
      ...
      any Grammar_SingleLineBody_produce(DEFAULT_ARGUMENTS){
       ...
       // 
        ...
        //for each item in .statements
        any _list86=PROP(statements_,this);
        { var item=undefined;
        for(int item__inx=0 ; item__inx<_list86.value.arr->length ; item__inx++){item=ITEM(item__inx,_list86);
...
//### Append to class Grammar.IfStatement ###
      ...
      any Grammar_IfStatement_produce(DEFAULT_ARGUMENTS){
        ...
        if (_anyToBool(PROP(elseStatement_,this)))  {
            ...
            METHOD(outSourceLinesAsComment_,this)(this,2,(any_arr){any_number(0), any_number(_anyToNumber(PROP(sourceLineNum_,PROP(elseStatement_,this))) - 1)});
...
//start body
        ...
        if (_anyToBool(PROP(paramsDeclarations_,this)) && _length(PROP(paramsDeclarations_,this)))  {
                ...
                //else
                    ...
                    any _list87=PROP(paramsDeclarations_,this);
                    { var paramDecl=undefined;
                    for(int paramDecl__inx=0 ; paramDecl__inx<_list87.value.arr->length ; paramDecl__inx++){paramDecl=ITEM(paramDecl__inx,_list87);
...
//we produce as chained if-else, using == to switchValue
        ...
        if (_anyToBool(PROP(varRef_,this)))  {
            ...
            for(int index=0 ; index<_list88.value.arr->length ; index++){switchCase=ITEM(index,_list88);
                ...
                METHOD(outSourceLineAsComment_,this)(this,1,(any_arr){PROP(sourceLineNum_,switchCase)});
                //.out 
                    //index>0? 'else ' : '' 
                ...
                METHOD(out_,this)(this,7,(any_arr){index > 0 ? any_str("else ") : any_EMPTY_STR, any_str("if ("), new(Map,4,(any_arr){
                    _newPair("pre",_concatAny(3,any_str("__is("), switchVar, any_str(","))), 
...
//with the casee expresions
        //else
          ...
          { var switchCase=undefined;
          for(int index=0 ; index<_list89.value.arr->length ; index++){switchCase=ITEM(index,_list89);
...
//### Append to class Grammar.SwitchCase ###
      ...
      any Grammar_SwitchCase_produce(DEFAULT_ARGUMENTS){
        ...
        any _list90=PROP(expressions_,this);
        { var expression=undefined;
        for(int expression__inx=0 ; expression__inx<_list90.value.arr->length ; expression__inx++){expression=ITEM(expression__inx,_list90);
        ...
        if (_anyToBool(PROP(varRef_,this)))  {
            ...
            for(int caseWhenSection__inx=0 ; caseWhenSection__inx<_list91.value.arr->length ; caseWhenSection__inx++){caseWhenSection=ITEM(caseWhenSection__inx,_list91);
                ...
                METHOD(out_,caseWhenSection)(caseWhenSection,6,(any_arr){any_str("if("), new(Map,4,(any_arr){
                    _newPair("pre",_concatAny(2,caseVar, any_str("==("))), 
...
//else, it's a var-less case. we code it as chained ternary operators
        //else
          ...
          { var caseWhenSection=undefined;
          for(int caseWhenSection__inx=0 ; caseWhenSection__inx<_list92.value.arr->length ; caseWhenSection__inx++){caseWhenSection=ITEM(caseWhenSection__inx,_list92);
          
              ////.outLineAsComment caseWhenSection.lineInx
              //.outSourceLineAsComment caseWhenSection.sourceLineNum
              METHOD(outSourceLineAsComment_,this)(this,1,(any_arr){PROP(sourceLineNum_,caseWhenSection)});
          ...
          METHOD(out_,this)(this,2,(any_arr){any_str("/* else */ "), (_anyToBool(__or27=PROP(elseExpression_,this))? __or27 : any_str("undefined"))});
        };
      return undefined;
      }
...
//Check location
     // 
        ...
        if (_anyToBool((_anyToBool(__or28=any_number(!(_anyToBool((functionDeclaration=METHOD(getParent_,this)(this,1,(any_arr){Grammar_FunctionDeclaration}))))))? __or28 : any_number(!_anyToBool(METHOD(hasAdjective_,functionDeclaration)(functionDeclaration,1,(any_arr){any_str("nice")}))))))  {
...
//Many LiteScript operators can be easily mapped one-to-one with their JavaScript equivalents.
   ...
   // 
    ...
    any Producer_c_operTranslate(DEFAULT_ARGUMENTS){
      ...
      return (_anyToBool(__or29=METHOD(get_,Producer_c_OPER_TRANSLATION_map)(Producer_c_OPER_TRANSLATION_map,1,(any_arr){name}))? __or29 : name);
    return undefined;
    }
//---------------------------------
//### Append to class ASTBase
