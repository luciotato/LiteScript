...
void litec__moduleInit(void){
   ...
   if (_anyToBool(CALL2(option_,litec_args,any_str("vers"), any_str("version"))))  {
       // print VERSION
   ...
   if (_anyToBool((litec_mainModuleName=CALL2(valueFor_,litec_args,any_str("r"), any_str("run")))))  {
       ...
       litec_compileAndRunParams = CALL1(splice_,PROP(items_,litec_args),PROP(lastIndex_,litec_args));// #remove params after --run
   };
   // with options
   var litec__with1=litec_options;
       // .outDir  = path.resolve(args.valueFor('o') or './out') //output dir
       PROP(outDir_,litec__with1) = path_resolve(undefined,1,(any_arr){__or(CALL1(valueFor_,litec_args,any_str("o")),any_str("./out"))}); //output dir
       // .verboseLevel = parseInt(args.valueFor('v',"verbose") or 0)
       PROP(verboseLevel_,litec__with1) = parseInt(undefined,1,(any_arr){__or(CALL2(valueFor_,litec_args,any_str("v"), any_str("verbose")),any_number(0))});
       // .warningLevel = parseInt(args.valueFor('w',"warning") or 1)
       PROP(warningLevel_,litec__with1) = parseInt(undefined,1,(any_arr){__or(CALL2(valueFor_,litec_args,any_str("w"), any_str("warning")),any_number(1))});
       // .comments= parseInt(args.valueFor('comment',"comments") or 1)
       PROP(comments_,litec__with1) = parseInt(undefined,1,(any_arr){__or(CALL2(valueFor_,litec_args,any_str("comment"), any_str("comments")),any_number(1))});
   ...
   while(_anyToBool((litec_newDef=CALL1(valueFor_,litec_args,any_str("D"))))){
       // options.defines.push newDef
       CALL1(push_,PROP(defines_,litec_options),litec_newDef);
   };// end loop
   ...
   if (_length(PROP(items_,litec_args)) > 1)  {
       ...
       print(2,(any_arr){any_str("Invalid arguments:"), CALL1(join_,PROP(items_,litec_args),any_str(" "))});
