...
// check for title-keywords: e.g.: `### Class MyClass`, `### export Function compile(sourceLines:string array)`
        ...
        if (_anyToNumber(countAdj) > 1 && !_anyToBool(countSust)) {METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(2,(any_arr){any_str("MarkDown Title-keyword, expected a sustantive: "), METHOD(join_,sustantives)(sustantives,0,NULL)})});};
        ...
        if (_anyToBool(countSust))  {
            ...
            int64_t _end5=_anyToNumber(inx);
            for(int64_t recogn=1; recogn<=_end5; recogn++) {
...
// Here we search for simple macros as __DATE__, __TIME__ , __TMESTAMP__
        ...
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
            ...
            line = METHOD(replaceAll_,line)(line,2,(any_arr){_concatAny(3,(any_arr){any_str("__"), macro, any_str("__")}), value});
...
// ifdef, #ifndef, #else and #endif should be the first thing on the line
        ...
        do{
            ...
            if (!_anyToBool(METHOD(nextSourceLine_,this)(this,0,NULL))) {METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(2,(any_arr){any_str("EOF "), startRef})});};
            //line = .line
            ...
            if (_anyToNumber((indent=METHOD(countSpaces_,line)(line,0,NULL))) >= 0)  {
                ...
                if (__is(METHOD(charAt_,line)(line,1,(any_arr){any_number(0)}),any_str("#")) && !__is(METHOD(charAt_,line)(line,1,(any_arr){any_number(1)}),any_str("#")))  { //expected: "#else, #endif #end if"
                    ...
                    if (__is(_switch1,any_str("#else"))){
                            ...
                            if (!__is(METHOD(tryGet_,words)(words,1,(any_arr){any_number(1)}),any_str("if"))) {METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(4,(any_arr){any_str("expected '#end if', read '"), line, any_str("' "), startRef})});};
                            //endFound = true
                            endFound = true;
                            ...
                            METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(4,(any_arr){any_str("expected '#else/#end if', read '"), line, any_str("' "), startRef})});
                    };
                    //end switch
                ...
                //else
                    ...
                    if (!(_anyToBool(defValue))) {METHOD(replaceSourceLine_,this)(this,1,(any_arr){_concatAny(3,(any_arr){Strings_spaces(undefined,1,(any_arr){indent}), any_str("//"), line})});};
                };
                //end if
...
// Save current pos, and get next token
        ...
        logger_debug(undefined,3,(any_arr){any_str(">>>ADVANCE"), _concatAny(6,(any_arr){PROP(sourceLineNum_,this), any_str(":"), (_anyToBool(__or2=PROP(column_,PROP(token_,this)))? __or2 : any_number(0)), any_str(" ["), PROP(index_,this), any_str("]")}), __call(toString_,PROP(token_,this),0,NULL)});
...
// **throwErr** add lexer position and emit error (abort compilation)
        ...
        logger_throwControlled(undefined,1,(any_arr){_concatAny(3,(any_arr){METHOD(posToString_,this)(this,0,NULL), any_str(" "), msg})});
     return undefined;
     }
...
// split expressions
        ...
        while(TRUE){
            ...
            delimiterPos = METHOD(indexOf_,s)(s,2,(any_arr){_concatAny(2,(any_arr){delimiter, any_str("{")}), lastDelimiterPos});
            ...
            if (_anyToNumber(closerPos) < 0)  {
                ...
                METHOD(throwErr_,this)(this,1,(any_arr){_concatAny(4,(any_arr){any_str("unmatched '"), delimiter, any_str("{' at string: "), text})});
            };
...
// If there was no match, this is a bad token and we will abort compilation here.
            ...
            if (!_anyToBool(token))  {
                ...
                while(_length(errPosString) < _anyToNumber(colInx)){
                    ...
                    errPosString = _concatAny(2,(any_arr){errPosString, any_str(" ")});
                };// end loop
...
// with a call to core function "concat"
                ...
                //ifdef PROD_C
                    ...
                    var composed = new(Parser_InfoLine,5,(any_arr){lexer, Parser_LineTypes_CODE, PROP(column_,token), _concatAny(3,(any_arr){any_str("_concatAny("), METHOD(join_,parsed)(parsed,1,(any_arr){any_str(",")}), any_str(")")}), PROP(sourceLineNum_,this)});
...
// Else it's a single token. Add the token to result array
                    ...
                    msg = _concatAny(2,(any_arr){msg, METHOD(toString_,token)(token,0,NULL)});
                    //endif
...
// that sits between *two* operands in a `Expressions`.
  ...
  // ['OPER', /^[\?\:]/],
            ...
            //if "|#{chunk.slice(0,2)}|" in "|<>|>=|<=|>>|<<|!=|"
            if (CALL1(indexOf_,any_str("|<>|>=|<=|>>|<<|!=|"),_concatAny(3,(any_arr){any_str("|"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), any_number(2)}), any_str("|")})).value.number>=0)  {
...
// Identifier-like OPERs, as: 'and', 'not', 'is','or' are checked before concluding is an IDENTIFIER
  ...
  // ['OPER', /^(is|isnt|not|and|but|into|like|or|in|into|instance|instanceof|has|hasnt)\b/],
            ...
            if (_anyToBool((wordLetters=PMREX_whileRanges(undefined,3,(any_arr){chunk, any_number(0), any_str("A-Za-z_\x7F-\xFF")}))))  {
                ...
                if (CALL1(indexOf_,any_str("|is|isnt|not|and|but|into|like|or|in|into|instance|instanceof|has|hasnt|"),_concatAny(3,(any_arr){any_str("|"), METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0), wordLetters}), any_str("|")})).value.number>=0)  {
...
//--------------
    ...
    any Parser_LexerPos; //Class Parser_LexerPos
      ...
      any Parser_LexerPos_toString(DEFAULT_ARGUMENTS){
        ...
        return _concatAny(5,(any_arr){PROP(filename_,PROP(lexer_,this)), any_str(":"), PROP(sourceLineNum_,this), any_str(":"), any_number(_anyToNumber((_anyToBool(__or6=PROP(column_,PROP(token_,this)))? __or6 : any_number(0))) + 1)});
      return undefined;
      }
...
// check if startCode is in the line, if not found, exit
        ...
        if (_anyToNumber(__call(indexOf_,Strings_replaceQuoted(undefined,2,(any_arr){line, any_EMPTY_STR}),1,(any_arr){startCode})) < 0)  {
            //return #no 
            return ;// #no
        };
...
// read lines until endCode is found
        ...
        while(!(_anyToNumber((endCol=METHOD(indexOf_,line)(line,1,(any_arr){endCode}))) >= 0)){
            ...
            if (!_anyToBool(METHOD(nextSourceLine_,lexer)(lexer,0,NULL)))  {
                ...
                METHOD(sayErr_,lexer)(lexer,1,(any_arr){_concatAny(9,(any_arr){any_str("EOF while processing multiline "), startCode, any_str(" (started on "), PROP(filename_,lexer), any_str(":"), startSourceLine, any_str(":"), startCol, any_str(")")})});
                //return
                return ;
            };
...
// append text to line
        ...
        if (_anyToBool(text))  {
            //.currLine &= text
            PROP(currLine_,this)=_concatAny(2,(any_arr){PROP(currLine_,this),text});
            //.column += text.length
...
// send the current line
          ...
          if (_anyToBool((_anyToBool(__or7=PROP(currLine_,this))? __or7 : any_number(__is(PROP(currLine_,this),any_EMPTY_STR)))))  {
              ...
              if (_anyToBool(PROP(toHeader_,this)))  {
                ...
                __call(push_,PROP(hLines_,this),1,(any_arr){PROP(currLine_,this)});
              }
              //else
              
              else {
                //.lines.push .currLine
                __call(push_,PROP(lines_,this),1,(any_arr){PROP(currLine_,this)});
                //.lineNum++
