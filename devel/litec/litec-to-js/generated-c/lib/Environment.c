...
//#### constructor (info:ImportParameterInfo)
     void Environment_FileInfo__init(DEFAULT_ARGUMENTS){
        ...
        PROP(hasPath_,this) = any_number(__in(METHOD(charAt_,name)(name,1,(any_arr){any_number(0)
        ...
        PROP(isInterface_,this) = any_number(!__is(__call(indexOf_,PROP(sourcename_,this),1,(any_arr){any_LTR(".interface.")
        }),any_number(-1)));
        ...
        PROP(base_,this) = path_basename(undefined,2,(any_arr){name
        , PROP(extension_,this)
        });

        //#remove .lite from double extension .lite.md
        //if this.base.endsWith('.lite'), this.base=this.base.slice(0,-5)
        if (_anyToBool(__call(endsWith_,PROP(base_,this),1,(any_arr){any_LTR(".lite")
        }))) {PROP(base_,this) = __call(slice_,PROP(base_,this),2,(any_arr){any_number(0)
        , any_number(-5)
        });};
     }
...
//#### method outWithExtension(ext)
     any Environment_FileInfo_outWithExtension(DEFAULT_ARGUMENTS){
        ...
        return path_join(undefined,2,(any_arr){PROP(outDir_,this)
        , _concatAny(2,PROP(base_,this)
        , ext
        )
        });
     return undefined;
...
//we search the module 
        ...
        if (_anyToBool(PROP(createFile_,PROP(importInfo_,this))))  {
            ...
            full = path_join(undefined,2,(any_arr){PROP(projectDir_,Environment_options)
        ...
        //else
            ...
            var search = new(Array,1,(any_arr){importingModuleDir})
            ;
            ...
            //else
                ...
                //search.push 
                    ...
                    //path.join(options.projectDir,'../lib')
                    //path.join(options.projectDir,'../interfaces')
                METHOD(push_,search)(search,4,(any_arr){path_join(undefined,2,(any_arr){PROP(projectDir_,Environment_options)
                    , any_LTR("/lib")
                    })
                    , path_join(undefined,2,(any_arr){PROP(projectDir_,Environment_options)
                    , any_LTR("/interfaces")
                    })
                    , path_join(undefined,2,(any_arr){PROP(projectDir_,Environment_options)
                    , any_LTR("../lib")
...
//gets redirected to dir `ENV_C_global_import` or `../ENV_C_global_import`
                ...
                if (__is(PROP(target_,Environment_options),any_LTR("c")))  {
                    ...
                    //search.unshift 
                        ...
                        //path.join(options.projectDir,'../interfaces/C_standalone')
                    METHOD(unshift_,search)(search,2,(any_arr){path_join(undefined,2,(any_arr){PROP(projectDir_,Environment_options)
                        , any_LTR("/interfaces/C_standalone")
                        })
                        , path_join(undefined,2,(any_arr){PROP(projectDir_,Environment_options)
                        , any_LTR("../interfaces/C_standalone")
                        })
...
//include command-line (-i foo/bar) specified dirs
                ...
                any _list64=PROP(includeDirs_,Environment_options);
                { var extra=undefined;
                for(int extra__inx=0 ; extra__inx<_list64.value.arr->length ; extra__inx++){extra=ITEM(extra__inx,_list64);
                
                    //search.push path.resolve(options.projectDir,extra)
                    METHOD(push_,search)(search,1,(any_arr){path_resolve(undefined,2,(any_arr){PROP(projectDir_,Environment_options)
                    , extra
                    })
                    });
                }};// end for each in
...
//add compiler directory to "include dirs" so GlobalScopeX.interface.md can be found
                ...
                //if __dirname 
                if (_anyToBool(__dirname))  {
                    //search.push path.resolve('#{__dirname}/../interfaces')
                    METHOD(push_,search)(search,1,(any_arr){path_resolve(undefined,1,(any_arr){_concatAny(2,__dirname
                    , any_LTR("/../interfaces")
                    )
                    })
                    });
...
//Now search the file in all specidief dirs/with all the extensions
            ...
            any _list65=search;
            { var dir=undefined;
            for(int dir__inx=0 ; dir__inx<_list65.value.arr->length ; dir__inx++){dir=ITEM(dir__inx,_list65);
            
                //var fname = path.join(dir,this.importInfo.name)
                var fname = path_join(undefined,2,(any_arr){dir
                , PROP(name_,PROP(importInfo_,this))
                })
                ;
                //for each ext in ['.lite.md','.md','.interface.md','.js'] 
                any _list66=new(Array,4,(any_arr){any_LTR(".lite.md"), any_LTR(".md"), any_LTR(".interface.md"), any_LTR(".js")});
                { var ext=undefined;
                for(int ext__inx=0 ; ext__inx<_list66.value.arr->length ; ext__inx++){ext=ITEM(ext__inx,_list66);
                
                    //full = fname & ext
                    full = _concatAny(2,fname,ext);
                    //if fs.existsSync(full)
                    if (_anyToBool(fs_existsSync(undefined,1,(any_arr){full
                    })))  {
                        //found=full
                        found = full;
                        //break
            ...
            if (!(_anyToBool(found)))  {
                ...
                logger_throwControlled(undefined,1,(any_arr){_concatAny(8,PROP(source_,PROP(importInfo_,this))
                , any_LTR(":")
                , PROP(line_,PROP(importInfo_,this))
                , any_LTR(":1 Module not found: ")
                , PROP(name_,PROP(importInfo_,this))
                , any_LTR("\nSearched as:\n")
                , (METHOD(join_,search)(search,1,(any_arr){any_LTR("\n")
                }))
        ...
        PROP(filename_,this) = path_resolve(undefined,1,(any_arr){full
        }); //full path
        ...
        PROP(sourcename_,this) = path_basename(undefined,1,(any_arr){PROP(filename_,this)
        });// #with extensions
        //this.isInterface = this.sourcename.indexOf('.interface.') isnt -1
        PROP(isInterface_,this) = any_number(!__is(__call(indexOf_,PROP(sourcename_,this),1,(any_arr){any_LTR(".interface.")
        }),any_number(-1)));
        //this.relPath = path.relative(options.projectDir, this.dir); //relative to basePath
        PROP(relPath_,this) = path_relative(undefined,2,(any_arr){PROP(projectDir_,Environment_options)
        , PROP(dir_,this)
        }); //relative to basePath
        //this.relFilename = path.relative(options.projectDir, this.filename); //relative to basePath
        PROP(relFilename_,this) = path_relative(undefined,2,(any_arr){PROP(projectDir_,Environment_options)
        , PROP(filename_,this)
        }); //relative to basePath
        ...
        PROP(outExtension_,this) = !(_anyToBool(PROP(isLite_,this))) && _anyToBool(PROP(extension_,this)) ? PROP(extension_,this) : _concatAny(2,any_LTR(".")
        , PROP(target_,Environment_options)
        );

        //this.outDir = path.join(options.outDir, this.relPath)
        PROP(outDir_,this) = path_join(undefined,2,(any_arr){PROP(outDir_,Environment_options)
        , PROP(relPath_,this)
        });
        //this.outFilename = path.join(this.outDir, "#{this.base}#{this.outExtension}");
        PROP(outFilename_,this) = path_join(undefined,2,(any_arr){PROP(outDir_,this)
        , _concatAny(2,PROP(base_,this)
        , PROP(outExtension_,this)
        )
        });
        //this.outRelFilename = path.relative(options.outDir, this.outFilename); //relative to options.outDir
        PROP(outRelFilename_,this) = path_relative(undefined,2,(any_arr){PROP(outDir_,Environment_options)
...
//check if the file exists, and if it is updated
        ...
        PROP(interfaceFile_,this) = path_join(undefined,2,(any_arr){PROP(dir_,this)
        , _concatAny(2,PROP(base_,this)
...
//Check if interface cache is updated
        ...
        if (_anyToBool(PROP(interfaceFileExists_,this)) && _anyToBool(isCacheFile))  {
            ...
            if (!(_anyToBool(PROP(interfaceFileExists_,this)))) {Environment_externalCacheSave(undefined,2,(any_arr){PROP(interfaceFile_,this)
            , null
            });};
        };
...
//### export Class ImportParameterInfo
    ...
    any Environment_setBaseInfo(DEFAULT_ARGUMENTS){
        // define named params
        var projectOptions= argc? arguments[0] : undefined;
        //---------
        Environment_options = projectOptions;
    return undefined;
    }
    any Environment_relativeFrom(DEFAULT_ARGUMENTS){
        ...
        //---------
        return path_relative(undefined,2,(any_arr){actualPath
        , destFilename
    ...
    any Environment_externalCacheSave(DEFAULT_ARGUMENTS){
        ...
        //else 
            ...
            if (_instanceof(fileLines,Array))  {
                ...
                fileLines = METHOD(join_,fileLines)(fileLines,1,(any_arr){any_LTR("\n")
                });
            };
            fs_writeFileSync(undefined,2,(any_arr){filename
            , fileLines
            });
        };
    return undefined;
    ...
    any Environment_fileInfoNewFile(DEFAULT_ARGUMENTS){
        ...
        ;
        METHOD(searchModule_,fileInfo)(fileInfo,1,(any_arr){null
        });
        return fileInfo;
    return undefined;
    }
