...
//--------------
    ...
    //auto Project_newFromObject
    inline any Project_newFromObject(DEFAULT_ARGUMENTS){
        return _newFromObject(Project,argc,arguments);
    }
...
//set basePath from main module path
        ...
        var tempFileInfo = new(Environment_FileInfo,1,(any_arr){PROP(mainModuleName_,options)
        })
        ;
        ...
        PROP(outDir_,options) = Environment_resolvePath(undefined,1,(any_arr){(_anyToBool(__or1=PROP(outDir_,options))? __or1 : any_LTR("."))
        });

        //Environment.setBaseInfo options
        Environment_setBaseInfo(undefined,1,(any_arr){options
        });

        //logger.msg 'Project Dir:',.options.projectDir
        logger_msg(undefined,2,(any_arr){any_LTR("Project Dir:")
        , PROP(projectDir_,PROP(options_,this))
        });
        //logger.msg 'Main Module:',.options.mainModuleName
        logger_msg(undefined,2,(any_arr){any_LTR("Main Module:")
        , PROP(mainModuleName_,PROP(options_,this))
        });
        //logger.msg 'Out Dir:',.options.outDir
        logger_msg(undefined,2,(any_arr){any_LTR("Out Dir:")
...
//compiler vars, to use at conditional compilation
        ...
        any _list6=PROP(defines_,options);
        { var def=undefined;
        for(int def__inx=0 ; def__inx<_list6.value.arr->length ; def__inx++){def=ITEM(def__inx,_list6);
...
//-if not cached-, all dependent modules. 
        ...
        logger_msg(undefined,2,(any_arr){any_LTR("Compiling")
        , PROP(mainModuleName_,PROP(options_,this))
        });
        ...
        PROP(source_,importInfo) = any_LTR("Project");
        //importInfo.line=0
        PROP(line_,importInfo) = any_number(0);
...
//Produce, for each module
        ...
        logger_msg(undefined,1,(any_arr){_concatAny(2,any_LTR("Producing ")
        ...
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
            ...
            if (_anyToBool((_anyToBool(__or2=PROP(isCore_,PROP(fileInfo_,moduleNode)))? __or2 : any_number(!_anyToBool(PROP(referenceCount_,moduleNode))))))  {
                //shouldProduce = false
                shouldProduce = false;
            };

            //if moduleNode.lexer.interfaceMode and .options.target is 'js'            
            if (_anyToBool(PROP(interfaceMode_,PROP(lexer_,moduleNode))) && __is(PROP(target_,PROP(options_,this)),any_LTR("js")))  {
            ...
            if (_anyToBool(shouldProduce))  {
                ...
                if (!(_anyToBool(PROP(isLite_,PROP(fileInfo_,moduleNode)))))  {
                    ...
                    logger_msg(undefined,1,(any_arr){_concatAny(3,any_LTR("Note: non-lite module '")
                    , PROP(filename_,PROP(fileInfo_,moduleNode))
                    , any_LTR("' required")
                    )
                    });
                    //result = moduleNode.fileInfo.filename
                    result = PROP(filename_,PROP(fileInfo_,moduleNode));
                }
...
//produce & get result target code
                ...
                logger_info(undefined,6,(any_arr){color_green
                , any_LTR("[OK]")
                , result
                , any_LTR(" -> ")
        ...
        logger_msg(undefined,1,(any_arr){_concatAny(6,any_LTR("Generated .")
        , PROP(target_,PROP(options_,this))
        , any_LTR(" files (")
...
//Called to compile GlobalScopeX.interface.md, from Validate module
        ...
        PROP(source_,PROP(importInfo_,filenameInfo)) = any_LTR("Compiler");
        //filenameInfo.importInfo.line=0
        PROP(line_,PROP(importInfo_,filenameInfo)) = any_number(0);
...
//Load source -> Lexer/Tokenize -> Parse/create AST 
        ...
        logger_info(undefined,2,(any_arr){String_spaces(undefined,1,(any_arr){any_number(_anyToNumber(PROP(recurseLevel_,this)) * 2)
        })
...
//Handle errors, add stage info, and stack
        ...
        //exception err
            ...
            if (_instanceof(err,ControlledError))  { //if not 'controlled' show lexer pos & call stack (includes err text)
                ...
                err = (_anyToBool(__or3=PROP(hardError_,PROP(lexer_,moduleNode)))? __or3 : err); //get important (inner) error
            }
            //else
...
//Check if this module 'imported other modules'. Process Imports (recursive)
        ...
        any _list8=PROP(requireCallNodes_,moduleNode);
        { var node=undefined;
        for(int node__inx=0 ; node__inx<_list8.value.arr->length ; node__inx++){node=ITEM(node__inx,_list8);
            ...
            PROP(line_,importInfo) = PROP(sourceLineNum_,node);
...
//if we're generating c-code, a interface or file must exist
        ...
        if (__is(PROP(target_,PROP(options_,this)),any_LTR("c"))) {return undefined;};
     return undefined;
     }
...
//#### helper method redirectOutput(newOut)
     any Project_redirectOutput(DEFAULT_ARGUMENTS){
        ...
        //for each moduleNode:Grammar.Module in map .moduleCache
        any _list9=PROP(moduleCache_,this);
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        int64_t __len=MAPSIZE(_list9); //how many pairs
        var moduleNode=undefined; //value
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
              __nvp = MAPITEM( __inx,_list9);
              moduleNode= __nvp->value;
