...
//Now, after processing all lines, we tokenize each CODE line
        ...
        any _list14=PROP(infoLines_,this);
        { var item=undefined;
...
//Get section between """ and """
        ...
        if (_anyToBool(PROP(section_,result)))  {
          ...
          for(int sectionLine1__inx=0 ; sectionLine1__inx<_list16.value.arr->length ; sectionLine1__inx++){sectionLine1=ITEM(sectionLine1__inx,_list16);
          ...
          for(int inx=0 ; inx<_list17.value.arr->length ; inx++){sectionLine=ITEM(inx,_list17);
...
//Now we should escape internal d-quotes, but only *outside* string interpolation expressions
          ...
          any _list18=parsed;
          { var item=undefined;
          for(int inx=0 ; inx<_list18.value.arr->length ; inx++){item=ITEM(inx,_list18);
              ...
              if (__is(METHOD(charAt_,item)(item,1,(any_arr){any_number(0)
                  ...
                  item = METHOD(replaceAll_,item)(item,2,(any_arr){any_LTR("\"")
                  , any_LTR("\\\"")
                  });// #store with *escaped* internal d-quotes
                  //parsed[inx] = '"#{item}"' #restore enclosing quotes
                  ITEM(inx,parsed) = _concatAny(3,any_LTR("\"")
                  , item
                  , any_LTR("\"")
      ...
      //else //  compile-to-js
          ...
          if (_length(parsed) && !__is(__call(charAt_,ITEM(0,parsed),1,(any_arr){any_number(0)
          }),any_LTR("\"")))  {
          ...
          //line = parsed.join(' + ')
          line = METHOD(join_,parsed)(parsed,1,(any_arr){any_LTR(" + ")
          });
          // add pre & post
      //endif
...
//This method handles multiline comments: `/*` `*/` 
        ...
        //else 
          ...
          //for each inx,sectionLine in result.section
          any _list19=PROP(section_,result);
          { var sectionLine=undefined;
          for(int inx=0 ; inx<_list19.value.arr->length ; inx++){sectionLine=ITEM(inx,_list19);
...
//Regex tokens are regular expressions. The javascript producer, just passes the raw regex to JavaScript.
  ...
  //['REGEX', /^(\/(?![\s=])[^[\/\n\\]*(?:(?:\\[\s\S]|\[[^\]\n\\]*(?:\\[\s\S][^\]\n\\]*)*])[^[\/\n\\]*)*\/)([imgy]{0,4})(?!\w)/],
            ...
            if (_anyToBool(METHOD(startsWith_,chunk)(chunk,1,(any_arr){any_LTR("/")
            })) && !__is(METHOD(indexOf_,chunk)(chunk,2,(any_arr){any_LTR("/")
            , any_number(1)
            }),any_number(-1)))  {
                ...
                var regexpExpr = METHOD(slice_,chunk)(chunk,2,(any_arr){any_number(0)
                , any_number(_length(regexpContents) + 2)
                })
                ; //include quote-chars: / & /
                //var regexpFlags = PMREX.whileRanges(chunk.slice(regexpExpr.length),"gimy")
                var regexpFlags = PMREX_whileRanges(undefined,2,(any_arr){METHOD(slice_,chunk)(chunk,1,(any_arr){any_number(_length(regexpExpr))
                })
                , any_LTR("gimy")
                })
                ;
                ...
                , _concatAny(2,regexpExpr,regexpFlags)
                });
            };
...
//send the current line
          ...
          if (_length(PROP(currLine_,this)))  {
              ...
              if (_anyToBool(PROP(fileMode_,this)))  {
                  ...
                  any _list20=PROP(currLine_,this);
                  { var part=undefined;
                  for(int part__inx=0 ; part__inx<_list20.value.arr->length ; part__inx++){part=ITEM(part__inx,_list20);
              ...
              //else
                  ...
                  __call(push_,ITEM(_anyToNumber(PROP(header_,this)),PROP(lines_,this)),1,(any_arr){__call(join_,PROP(currLine_,this),1,(any_arr){any_EMPTY_STR
                  })
                  });
              };
...
//#### Method blankLine()
     any Parser_OutCode_blankLine(DEFAULT_ARGUMENTS){
          ...
          METHOD(startNewLine_,this)(this,0,NULL);
          //.currLine.push ""
          __call(push_,PROP(currLine_,this),1,(any_arr){any_EMPTY_STR
          });
          //.startNewLine
          METHOD(startNewLine_,this)(this,0,NULL);
     return undefined;
...
//get result and clear memory      
        ...
        //return .lines[header]
        return ITEM(_anyToNumber(header),PROP(lines_,this));
     return undefined;
     }
