...
//with the class, or match the token type to the string.
        ...
        any _list10=_newArray(argc,arguments);
        { var searched=undefined;
        for(int searched__inx=0 ; searched__inx<_list10.value.arr->length ; searched__inx++){searched=ITEM(searched__inx,_list10);
...
//optional newline after comma 
            ...
            consumedNewLine = METHOD(opt_,this)(this,1,(any_arr){optSepar
            });
            ...
            if (_anyToBool(consumedNewLine) && _anyToNumber(PROP(indent_,PROP(lexer_,this))) <= _anyToNumber(blockIndent))  {
                ...
                __call(sayErr_,PROP(lexer_,this),1,(any_arr){_concatAny(7,any_LTR("SeparatedList, after '")
                , separator
...
//and syntax error reporting
        ...
        any _list11=args;
        { var i=undefined;
        for(int i__inx=0 ; i__inx<_list11.value.arr->length ; i__inx++){i=ITEM(i__inx,_list11);
...
//It evaluates and output its arguments. uses .lexer.out
        ...
        any _list12=_newArray(argc,arguments);
        { var item=undefined;
        for(int item__inx=0 ; item__inx<_list12.value.arr->length ; item__inx++){item=ITEM(item__inx,_list12);
...
//{CSL:arr} -> output the array as Comma Separated List (note: CSL can be present and undefined)
              ...
              if (_anyToBool(METHOD(hasProperty_,item)(item,1,(any_arr){any_LTR("CSL")
                  ...
                  if (_anyToBool(CSL))  {
                      ...
                      for(int inx=0 ; inx<_list13.value.arr->length ; inx++){listItem=ITEM(inx,_list13);
...
//Note: check if we can remove "outLineAsComment" and use this instead
        ...
        //else
            ...
            if (!__is(METHOD(substr_,line)(line,2,(any_arr){indent
            , any_number(2)
            }),any_LTR("//"))) {__call(put_,PROP(outCode_,PROP(lexer_,this)),1,(any_arr){any_LTR("//")
            });};
