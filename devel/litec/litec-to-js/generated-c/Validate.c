...
//set module.exports with default export object if set
        ...
        any _list35=PROP(moduleCache_,Validate_project);
...
//Make var x point to imported module 'x' exports 
        ...
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
          ...
          any _list37=PROP(requireCallNodes_,moduleNode);
          { var node=undefined;
          for(int node__inx=0 ; node__inx<_list37.value.arr->length ; node__inx++){node=ITEM(node__inx,_list37);
...
//'importedModule.exports' points to the class 'prototype'. 
                  ...
                  if (!__is(METHOD(getParent_,node)(node,1,(any_arr){Grammar_DeclareStatement
                        ...
                        //else
                            ...
                            any _list38=PROP(members_,moveWhat);
                            {NameValuePair_ptr __nvp=NULL; //name:value pair
                            int64_t __len=MAPSIZE(_list38); //how many pairs
                            var nameDecl=undefined; //value
                            for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
                                __nvp = MAPITEM( __inx,_list38);
...
//For all modules, for each node, if the specific AST node has methodName, call it
        ...
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        int64_t __len=MAPSIZE(_list43); //how many pairs
        var moduleNode=undefined; //value
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
            __nvp = MAPITEM( __inx,_list43);
            moduleNode= __nvp->value;
        ...
        if (_anyToBool(PROP(nodeDeclared_,this)) && !(_anyToBool(String_isCapitalized(undefined,1,(any_arr){PROP(name_,this)
            ...
            if (!(_anyToBool(METHOD(findOwnMember_,this)(this,1,(any_arr){any_LTR("**proto**")
                ...
                if (!(_anyToBool(possibleClassRef)) && _length(PROP(name_,this)) >= 6)  {
                    ...
                    any _list45=PROP(members_,Validate_nameAffinity);
                    {NameValuePair_ptr __nvp=NULL; //name:value pair
...
, excludeClass
            ...
            //else if value instance of Array
                ...
                any _list47=value;
                { var item=undefined;
                for(int item__inx=0 ; item__inx<_list47.value.arr->length ; item__inx++){item=ITEM(item__inx,_list47);
...
//search for a export default object (a class/namespace named as the module)
      ...
      {NameValuePair_ptr __nvp=NULL; //name:value pair
      int64_t __len=MAPSIZE(_list48); //how many pairs
      var nameDecl=undefined; //value
      for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
          __nvp = MAPITEM( __inx,_list48);
          nameDecl= __nvp->value;
...
//we check for duplicate property names in the super-class-chain
        ...
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        int64_t __len=MAPSIZE(_list52); //how many pairs
        var propNameDecl=undefined; //value
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
                __nvp = MAPITEM( __inx,_list52);
...
//add parameters to function's scope
      ...
      if (_anyToBool(PROP(paramsDeclarations_,this)))  {
          ...
          any _list53=PROP(paramsDeclarations_,this);
          { var varDecl=undefined;
...
, any_LTR("' has no prototype")
      ...
      any _list54=PROP(statements_,PROP(body_,this));
      { var item=undefined;
      for(int item__inx=0 ; item__inx<_list54.value.arr->length ; item__inx++){item=ITEM(item__inx,_list54);
...
//Add all properties as members of its owner object (normally: class.prototype)
        ...
        if (_anyToBool((ownerNameDecl=METHOD(tryGetOwnerNameDecl_,this)(this,1,(any_arr){informError
            ...
            { var varDecl=undefined;
            for(int varDecl__inx=0 ; varDecl__inx<_list55.value.arr->length ; varDecl__inx++){varDecl=ITEM(varDecl__inx,_list55);
...
//now follow each accessor
        ...
        any _list57=PROP(accessors_,this);
        { var ac=undefined;
        for(int ac__inx=0 ; ac__inx<_list57.value.arr->length ; ac__inx++){ac=ITEM(ac__inx,_list57);
...
, opt
            ...
            any _list61=PROP(accessors_,PROP(varRef_,this));
            { var ac=undefined;
            for(int ac__inx=0 ; ac__inx<_list61.value.arr->length ; ac__inx++){ac=ITEM(ac__inx,_list61);
