...
//set module.exports with default export object if set
        ...
        any _list36=PROP(moduleCache_,Validate_project);
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        int64_t __len=MAPSIZE(_list36); //how many pairs
        var moduleNode=undefined; //value
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
            __nvp = MAPITEM( __inx,_list36);
...
//Make var x point to imported module 'x' exports 
        ...
        //for each moduleNode:Grammar.Module in map project.moduleCache
        any _list37=PROP(moduleCache_,Validate_project);
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        int64_t __len=MAPSIZE(_list37); //how many pairs
        var moduleNode=undefined; //value
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
          __nvp = MAPITEM( __inx,_list37);
          moduleNode= __nvp->value;
        

          //for each node in moduleNode.requireCallNodes
          any _list38=PROP(requireCallNodes_,moduleNode);
          { var node=undefined;
          for(int node__inx=0 ; node__inx<_list38.value.arr->length ; node__inx++){node=ITEM(node__inx,_list38);
...
//'importedModule.exports' points to the class 'prototype'. 
                  ...
                  if (!__is(METHOD(getParent_,node)(node,1,(any_arr){Grammar_DeclareStatement
                        ...
                        //else
                            ...
                            any _list39=PROP(members_,moveWhat);
                            {NameValuePair_ptr __nvp=NULL; //name:value pair
...
//#### Pass 2.0 Apply Name Affinity
        ...
        any _list40=Names_allNameDeclarations;
...
//Repeat until no conversions can be made.
        ...
        do{
            ...
            //for each nameDecl in Names.allNameDeclarations 
            any _list41=Names_allNameDeclarations;
            { var nameDecl=undefined;
...
//Inform unconverted types as errors
        ...
        if (_anyToBool(sumFailures))  {// #there was failures, inform al errors
            ...
            { var nameDecl=undefined;
            for(int nameDecl__inx=0 ; nameDecl__inx<_list42.value.arr->length ; nameDecl__inx++){nameDecl=ITEM(nameDecl__inx,_list42);
...
//Inform forward declarations not fulfilled, as errors
        ...
        //for each nameDecl in Names.allNameDeclarations
        any _list43=Names_allNameDeclarations;
        { var nameDecl=undefined;
        for(int nameDecl__inx=0 ; nameDecl__inx<_list43.value.arr->length ; nameDecl__inx++){nameDecl=ITEM(nameDecl__inx,_list43);
...
//### export function walkAllNodesCalling(methodName:string)
    any Validate_walkAllNodesCalling(DEFAULT_ARGUMENTS){
        ...
        if (__is(PROP(size_,PROP(moduleCache_,Validate_project)),any_number(0)))  {
            //throw new Error("walkAllNodes: no modules in moduleCache")
            throw(new(Error,1,(any_arr){any_LTR("walkAllNodes: no modules in moduleCache")
            }));
        };
...
//For all modules, for each node, if the specific AST node has methodName, call it
        ...
        //for each moduleNode:Grammar.Module in map project.moduleCache
        any _list44=PROP(moduleCache_,Validate_project);
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        int64_t __len=MAPSIZE(_list44); //how many pairs
        var moduleNode=undefined; //value
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
            __nvp = MAPITEM( __inx,_list44);
            moduleNode= __nvp->value;
...
//b.2) Lite-C: the Lexer replaces string interpolation with calls to `_concatAny`
        ...
        METHOD(addMember_,Validate_globalScope)(Validate_globalScope,1,(any_arr){any_LTR("__dirname")
        }); // current module dir (node.js)
...
//Process the global scope declarations interface file: GlobalScope(JS|C|NODE).interface.md
        ...
        logger_msg(undefined,1,(any_arr){_concatAny(3,any_LTR("Declare global scope using ")
        , globalInterfaceFile
        ...
        if (__is(PROP(verboseLevel_,PROP(options_,Validate_project)),any_number(1))) {print(2,(any_arr){any_LTR("from:"), PROP(relFilename_,PROP(fileInfo_,globalInterfaceModule))});};
...
//move all exported from the interface file, to project.rootModule global scope
        ...
        any _list45=PROP(members_,PROP(exports_,globalInterfaceModule));
        {NameValuePair_ptr __nvp=NULL; //name:value pair
...
//Add a built-in class to global scope, return class prototype
      ...
      if (!(_anyToBool((classProto=METHOD(findOwnMember_,nameDecl)(nameDecl,1,(any_arr){any_LTR("prototype")
          ...
          throw(new(Error,1,(any_arr){_concatAny(3,any_LTR("addBuiltInClass '")
          , name
...
//Add a built-in object to global scope, return object
      ...
      if (_anyToBool(METHOD(findOwnMember_,nameDecl)(nameDecl,1,(any_arr){any_LTR("prototype")
          ...
          throw(new(Error,1,(any_arr){_concatAny(3,any_LTR("addBuiltObject '")
          , name
        ...
        if (_anyToBool(PROP(nodeDeclared_,this)) && !(_anyToBool(String_isCapitalized(undefined,1,(any_arr){PROP(name_,this)
            ...
            if (!(_anyToBool(METHOD(findOwnMember_,this)(this,1,(any_arr){any_LTR("**proto**")
                ...
                if (!(_anyToBool(possibleClassRef)) && _length(PROP(name_,this)) >= 6)  {
                    ...
                    any _list46=PROP(members_,Validate_nameAffinity);
                    {NameValuePair_ptr __nvp=NULL; //name:value pair
     ...
     any ASTBase_addToSpecificScope(DEFAULT_ARGUMENTS){
        ...
        if (_anyToBool((found=METHOD(findInScope_,this)(this,1,(any_arr){name
            ...
            //else 
                ...
                var errPosNode = _instanceof(item,Names_Declaration) ? item : this
                ;
                //errPosNode.sayErr "DUPLICATED name in scope: '#{name}'"
                METHOD(sayErr_,errPosNode)(errPosNode,1,(any_arr){_concatAny(3,any_LTR("DUPLICATED name in scope: '")
                , name
                , any_LTR("'")
                )
                });
...
//search for a export default object (a class/namespace named as the module)
      ...
      any _list49=PROP(members_,PROP(exports_,this));
      {NameValuePair_ptr __nvp=NULL; //name:value pair
      ...
      if (_anyToBool(exportDefaultNameDecl))  {
          ...
          if (_anyToNumber(__call(getMemberCount_,PROP(exports_,this),0,NULL)) > 1)  {
              //for each nameDecl in map .exports.members
              any _list50=PROP(members_,PROP(exports_,this));
              {NameValuePair_ptr __nvp=NULL; //name:value pair
              int64_t __len=MAPSIZE(_list50); //how many pairs
              var nameDecl=undefined; //value
              for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
                  __nvp = MAPITEM( __inx,_list50);
                  nameDecl= __nvp->value;
          ...
          if (_anyToBool(PROP(nameDecl_,this)) && _anyToBool(PROP(assignedValue_,this)) && !__is(PROP(name_,PROP(nameDecl_,this)),any_LTR("prototype")))  {
              ...
              if (_instanceof(PROP(assignedValue_,this),Grammar_Expression) && __in(any_class(PROP(name_,PROP(root_,PROP(assignedValue_,this))).class),2,(any_arr){Grammar_StringLiteral, Grammar_NumberLiteral}))  {
                //and .assignedValue.root.name.constructor in [Grammar.StringLiteral,Grammar.NumberLiteral]
                    ...
                    var theLiteral = PROP(name_,PROP(root_,PROP(assignedValue_,this)))
                    ;
              ...
              , Validate_globalPrototype(undefined,1,(any_arr){PROP(type_,theLiteral)
                    })
              });
              }
              ...
              else if (!(_anyToBool((type=__call(findOwnMember_,PROP(nameDecl_,this),1,(any_arr){any_LTR("**proto**")
              })))))  {// #if has no type
...
//### Append to class Grammar.VarStatement ###
     ...
     any Grammar_VarStatement_declare(DEFAULT_ARGUMENTS){
        ...
        any _list51=PROP(list_,this);
        { var varDecl=undefined;
        for(int varDecl__inx=0 ; varDecl__inx<_list51.value.arr->length ; varDecl__inx++){varDecl=ITEM(varDecl__inx,_list51);
...
//'prototype' starts with 'constructor' which is a pointer to the class-funcion itself
        ...
        if (_anyToBool(isClass))  {
            ...
            if (_anyToBool(PROP(varRefSuper_,this)))  {
                ...
                METHOD(setMember_,prtypeNameDecl)(prtypeNameDecl,2,(any_arr){any_LTR("**proto**")
...
//we check for duplicate property names in the super-class-chain
        ...
        any _list53=PROP(members_,prt);
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        int64_t __len=MAPSIZE(_list53); //how many pairs
        var propNameDecl=undefined; //value
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
                __nvp = MAPITEM( __inx,_list53);
...
//so *it does not declare a type with members*
        ...
        if (__is(PROP(target_,PROP(options_,Validate_project)),any_LTR("js")))  {
...
//of a name:value pair)
          ...
          PROP(nameDecl_,this) = __call(tryGetProperty_,PROP(parent_,this),1,(any_arr){any_LTR("nameDecl")
          });

          //if no .nameDecl
          if (!_anyToBool(PROP(nameDecl_,this)))  {
              //.nameDecl = .declareName(UniqueID.getVarName('*ObjectLiteral*'))
              PROP(nameDecl_,this) = METHOD(declareName_,this)(this,1,(any_arr){UniqueID_getVarName(undefined,1,(any_arr){any_LTR("*ObjectLiteral*")
              })
              });
          };
        };
...
//but it does not declare a valid type/class.
        ...
        if (__is(PROP(target_,PROP(options_,Validate_project)),any_LTR("c"))) {return undefined;};
...
//Add this name as member of the parent ObjectLiteral/Value
        ...
        PROP(nameDecl_,this) = METHOD(addMemberTo_,this)(this,2,(any_arr){PROP(nameDecl_,PROP(parent_,this))
        , PROP(name_,this)
        });
...
//check if we can determine type from value 
        ...
        if (_anyToBool(PROP(type_,this)) && _instanceof(PROP(type_,this),Names_Declaration) && !(__in(PROP(name_,PROP(type_,this)),2,(any_arr){any_LTR("undefined"), any_LTR("null")})))  {
...
//add parameters to function's scope
      ...
      if (_anyToBool(PROP(paramsDeclarations_,this)))  {
          ...
          any _list54=PROP(paramsDeclarations_,this);
          { var varDecl=undefined;
...
//get referenced class/namespace
      ...
      any _list55=PROP(statements_,PROP(body_,this));
      { var item=undefined;
      for(int item__inx=0 ; item__inx<_list55.value.arr->length ; item__inx++){item=ITEM(item__inx,_list55);
...
//now check each accessor
        ...
        //for each ac in .accessors
        any _list59=PROP(accessors_,this);
        { var ac=undefined;
        for(int ac__inx=0 ; ac__inx<_list59.value.arr->length ; ac__inx++){ac=ITEM(ac__inx,_list59);
...
//if it is assigning string or number literal, force type 
      ...
      if (_instanceof(PROP(rvalue_,this),Grammar_Expression))  {
          ...
          if (__in(any_class(PROP(name_,PROP(root_,PROP(rvalue_,this))).class),2,(any_arr){Grammar_StringLiteral, Grammar_NumberLiteral}))  {
              ...
              var theLiteral = PROP(name_,PROP(root_,PROP(rvalue_,this)))
              ;
              // if it is assigning a literal, force type to string|number|array
              //reference.setMember('**proto**', globalPrototype(theLiteral.type))
              METHOD(setMember_,reference)(reference,2,(any_arr){any_LTR("**proto**")
              , Validate_globalPrototype(undefined,1,(any_arr){PROP(type_,theLiteral)
              })
              });
              //return
...
//declare valid x.y.z
      ...
      if (__is(PROP(specifier_,this),any_LTR("on")))  {
          ...
          any _list60=PROP(names_,this);
          { var varDecl=undefined;
          for(int varDecl__inx=0 ; varDecl__inx<_list60.value.arr->length ; varDecl__inx++){varDecl=ITEM(varDecl__inx,_list60);
...
//else: declare (name affinity|var) (VariableDecl,)
      ...
      //else if .specifier in ['affinity','var']
          ...
          //for each varDecl in .names
          any _list61=PROP(names_,this);
          { var varDecl=undefined;
          for(int varDecl__inx=0 ; varDecl__inx<_list61.value.arr->length ; varDecl__inx++){varDecl=ITEM(varDecl__inx,_list61);
...
//declare members on the fly, with optional type
      ...
      if (__is(PROP(specifier_,this),any_LTR("valid"))){
            ...
            any _list62=PROP(accessors_,PROP(varRef_,this));
            { var ac=undefined;
            for(int ac__inx=0 ; ac__inx<_list62.value.arr->length ; ac__inx++){ac=ITEM(ac__inx,_list62);
...
//### helper function AddGlobalClasses()
    any Validate_AddGlobalClasses(DEFAULT_ARGUMENTS){
        ...
        any _list63=_newArray(argc,arguments);
        { var name=undefined;
        for(int name__inx=0 ; name__inx<_list63.value.arr->length ; name__inx++){name=ITEM(name__inx,_list63);
