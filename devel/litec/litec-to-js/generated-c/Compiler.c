...
//* string, compiled code
        ...
        var 
        moduleNode = Compiler_compileModule(undefined,3,(any_arr){
        filename, 
        sourceLines, 
        options
})
...
//Create a 'Project' to hold the main module and dependant modules
        ...
        any_LTR("Total Compile Project")
});
        //var project = new Project(mainModule, options)
        var 
        project = new(Project,2,(any_arr){
        mainModule, 
        options
})
;
        ...
        if (_anyToBool(PROP(perf_,options)))  {
            ...
            console_timeEnd(undefined,1,(any_arr){
        any_LTR("Total Compile Project")
});
        };
        //return project
...
;
        ...
        var 
        moduleNode = METHOD(createNewModule_,project)(project,1,(any_arr){
        fileInfo
})
;
//parse source lines & store in moduleCache for validation
        ...
        METHOD(parseOnModule_,project)(project,3,(any_arr){
        moduleNode, 
        filename, 
        sourceLines
});
        //project.moduleCache.set filename,moduleNode
        __call(set_,PROP(moduleCache_,project),2,(any_arr){
        filename, 
        moduleNode
});
//import dependencies
        ...
        if (!_anyToBool(PROP(single_,PROP(options_,project))))  {
            ...
            METHOD(importDependencies_,project)(project,1,(any_arr){
        moduleNode
});
        };
//validate var & property names
        ...
        project
            ...
            if (__is(logger_errorCount,any_number(0))) {logger_info(undefined,1,(any_arr){
        any_LTR("Validation OK")
});};
        };
//initialize out buffer & produce target code 
        ...
        METHOD(produceModule_,project)(project,1,(any_arr){
        moduleNode
});
        ...
        if (!__is(logger_errorCount,any_number(0))) {logger_throwControlled(undefined,1,(any_arr){
        any_LTR("#logger.errorCount errors during compilation")
});};
...
void Compiler__namespaceInit(void){
    ...
    Compiler_buildDate = any_LTR("Wed Jul 30 2014 22:24:27 GMT-0300 (ART)");
};
