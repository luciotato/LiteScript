...
//we search the module 
        ...
        if (_anyToBool(PROP(createFile_,PROP(importInfo_,this))))  {
            ...
            full = path_join(undefined,2,(any_arr){Environment_projectDir
, PROP(name_,PROP(importInfo_,this))
        ...
        //else
            ...
            if (_anyToBool(PROP(hasPath_,this)))  {// #specific path indicated
                ...
                search = new(Array,1,(any_arr){path_resolve(undefined,2,(any_arr){importingModuleDir
, PROP(name_,PROP(importInfo_,this))
                })});
            }
            //else
                ...
                //search = [ 
                    ...
                    //path.join(projectDir,'../lib',this.importInfo.name)
                    //path.join(projectDir,'../interfaces',this.importInfo.name)
                search = new(Array,5,(any_arr){path_join(undefined,2,(any_arr){importingModuleDir
, PROP(name_,PROP(importInfo_,this))
                    }), path_join(undefined,3,(any_arr){Environment_projectDir
, any_LTR("/lib")
, PROP(name_,PROP(importInfo_,this))
                    }), path_join(undefined,3,(any_arr){Environment_projectDir
, any_LTR("/interfaces")
...
//gets redirected to dir `ENV_C_global_import` or `../ENV_C_global_import`
                ...
                if (__is(Environment_targetExt,any_LTR("c")))  {
                    ...
                    //search.unshift 
                        ...
                        //path.join(projectDir,'../ENV_C_global_import',this.importInfo.name)
                    METHOD(unshift_,search)(search,2,(any_arr){path_join(undefined,3,(any_arr){Environment_projectDir
...
//Now search the fille in all the locations/with all the extensions
            ...
            for(int item__inx=0 ; item__inx<_list61.value.arr->length ; item__inx++){item=ITEM(item__inx,_list61);
                ...
                any _list62=new(Array,4,(any_arr){any_LTR(".lite.md"), any_LTR(".md"), any_LTR(".interface.md"), any_LTR(".js")});
                { var ext=undefined;
                for(int ext__inx=0 ; ext__inx<_list62.value.arr->length ; ext__inx++){ext=ITEM(ext__inx,_list62);
                
                    //if fs.existsSync("#{item}#{ext}" into full)
                    if (_anyToBool(fs_existsSync(undefined,1,(any_arr){(full=_concatAny(2,item
, ext
                    ))
                    })))  {
                        //found=full
...
, any_LTR("\n   with extensions (.lite.md|.md|.interface.md|.js)")
        ...
        PROP(relPath_,this) = path_relative(undefined,2,(any_arr){Environment_projectDir
, PROP(dir_,this)
        }); //relative to basePath
        //this.relFilename = path.relative(projectDir, this.filename); //relative to basePath
        PROP(relFilename_,this) = path_relative(undefined,2,(any_arr){Environment_projectDir
, PROP(filename_,this)
        }); //relative to basePath
...
, Environment_targetExt
        ...
        PROP(outDir_,this) = path_join(undefined,2,(any_arr){Environment_outDir
, PROP(relPath_,this)
        });
...
, PROP(outExtension_,this)
        ...
        PROP(outRelFilename_,this) = path_relative(undefined,2,(any_arr){Environment_outDir
