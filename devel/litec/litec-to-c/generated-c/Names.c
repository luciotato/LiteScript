...
//mix in found namedecl here
        ...
        any _list30=PROP(members_,realNameDecl);
...
//#### helper method makePointTo(nameDecl:Declaration)
     any Names_Declaration_makePointTo(DEFAULT_ARGUMENTS){
        ...
        any _list31=PROP(members_,this);
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        int64_t __len=MAPSIZE(_list31); //how many pairs
        var memberDecl=undefined; //value
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
          __nvp = MAPITEM( __inx,_list31);
        ...
        any _list32=Names_allNameDeclarations;
        { var other=undefined;
        for(int other__inx=0 ; other__inx<_list32.value.arr->length ; other__inx++){other=ITEM(other__inx,_list32);
