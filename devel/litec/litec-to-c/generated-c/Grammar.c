...
var Grammar_unaryOperators;
    ...
    static _methodInfoArr Grammar_RegExpLiteral_METHODS = {
      ...
      { produce_, Grammar_RegExpLiteral_produce },
...
, PROP(name_,this)
        ...
        if (_anyToBool(PROP(accessors_,this)))  {
          ...
          any _list20=PROP(accessors_,this);
          { var ac=undefined;
          for(int ac__inx=0 ; ac__inx<_list20.value.arr->length ; ac__inx++){ac=ITEM(ac__inx,_list20);
...
//so `a = new MyClass` turns into `a = new MyClass()`
        ...
        any _list21=arr;
        { var item=undefined;
        for(int index=0 ; index<_list21.value.arr->length ; index++){item=ITEM(index,_list21);
...
//(on equal precedende, we use the leftmost)
          ...
          any _list22=arr;
          { var item=undefined;
          for(int inx=0 ; inx<_list22.value.arr->length ; inx++){item=ITEM(inx,_list22);
...
//####helper Functions
      ...
      any Grammar_ObjectLiteral_forEach(DEFAULT_ARGUMENTS){
          ...
          any _list23=PROP(items_,this);
          { var nameValue=undefined;
          for(int nameValue__inx=0 ; nameValue__inx<_list23.value.arr->length ; nameValue__inx++){nameValue=ITEM(nameValue__inx,_list23);
...
//Check validity of adjective-statement combination 
        ...
        any _list28=PROP(adjectives_,this);
        { var adjective=undefined;
        for(int adjective__inx=0 ; adjective__inx<_list28.value.arr->length ; adjective__inx++){adjective=ITEM(adjective__inx,_list28);
...
//this method check all the body statements againts a valid-list (arguments)      
        ...
        any _list29=PROP(statements_,this);
        { var stm=undefined;
        for(int stm__inx=0 ; stm__inx<_list29.value.arr->length ; stm__inx++){stm=ITEM(stm__inx,_list29);
