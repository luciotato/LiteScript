...
//launch project compilation
    ...
    any c_lite_startCompilation(DEFAULT_ARGUMENTS){ try{
        ...
        Compiler_compileProject(undefined,2,(any_arr){c_lite_mainModuleName
, c_lite_options
        });
...
void c_lite__moduleInit(void){
    ...
    c_lite_BUILD_DATE = any_LTR("2014-07-29T21:05:48.266Z");
...
, any_LTR("version")
    ...
    if (_anyToBool((c_lite_mainModuleName=METHOD(valueFor_,c_lite_args)(c_lite_args,2,(any_arr){any_LTR("r")
, any_LTR("run")
    }))))  {
        c_lite_compileAndRunOption = true;
        c_lite_compileAndRunParams = __call(splice_,PROP(items_,c_lite_args),1,(any_arr){PROP(lastIndex_,c_lite_args)
...
, any_LTR("debug")
    ...
    if (_anyToNumber(PROP(verboseLevel_,c_lite_options)) > 1)  {
        print(1,(any_arr){_concatAny(10,any_LTR("compiler version: ")
, Compiler_version
, any_LTR(" ")
, Compiler_buildDate
, any_LTR("\ncompiler options: \n")
