...
//Make var x point to imported module 'x' exports 
        //for each moduleNode:Grammar.Module in map project.moduleCache
        any _list34=PROP(moduleCache_,Validate_project);
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        ...
        for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
          ...
          any _list35=PROP(requireCallNodes_,moduleNode);
          { var node=undefined;
          for(int node__inx=0 ; node__inx<_list35.value.arr->length ; node__inx++){node=ITEM(node__inx,_list35);
...
//'importedModule.exports' points to the class 'prototype'. 
                  ...
                  if (!__is(METHOD(getParent_,node)(node,1,(any_arr){Grammar_DeclareStatement
                        ...
                        //else
                            ...
                            any _list36=PROP(members_,moveWhat);
                            {NameValuePair_ptr __nvp=NULL; //name:value pair
...
//For all modules, for each node, if the specific AST node has methodName, call it
        //for each moduleNode:Grammar.Module in map project.moduleCache
        any _list41=PROP(moduleCache_,Validate_project);
        {NameValuePair_ptr __nvp=NULL; //name:value pair
...
//move all exported from the interface file, to project.rootModule global scope
        ...
        any _list42=PROP(members_,PROP(exports_,globalInterfaceModule));
        {NameValuePair_ptr __nvp=NULL; //name:value pair
        ...
        if (_anyToBool(PROP(nodeDeclared_,this)) && !(_anyToBool(String_isCapitalized(undefined,1,(any_arr){PROP(name_,this)
            ...
            if (!(_anyToBool(METHOD(findOwnMember_,this)(this,1,(any_arr){any_LTR("**proto**")
                ...
                if (!(_anyToBool(possibleClassRef)) && _length(PROP(name_,this)) >= 6)  {
                    ...
                    any _list43=PROP(members_,Validate_nameAffinity);
                    {NameValuePair_ptr __nvp=NULL; //name:value pair
...
, excludeClass
            ...
            //else if value instance of Array
                ...
                any _list45=value;
                { var item=undefined;
                for(int item__inx=0 ; item__inx<_list45.value.arr->length ; item__inx++){item=ITEM(item__inx,_list45);
...
//search for a export default object (a class/namespace named as the module)
      ...
      if (_anyToBool(exportDefaultNameDecl))  {
          ...
          if (_anyToNumber(__call(getMemberCount_,PROP(exports_,this),0,NULL)) > 1)  {
              //for each nameDecl in map .exports.members
              any _list47=PROP(members_,PROP(exports_,this));
              {NameValuePair_ptr __nvp=NULL; //name:value pair
              int64_t __len=MAPSIZE(_list47); //how many pairs
              var nameDecl=undefined; //value
              for(int64_t __inx=0 ; __inx < __len ; __inx++ ){
                  __nvp = MAPITEM( __inx,_list47);
                  nameDecl= __nvp->value;
...
//add parameters to function's scope
      ...
      if (_anyToBool(PROP(paramsDeclarations_,this)))  {
          ...
          any _list51=PROP(paramsDeclarations_,this);
          { var varDecl=undefined;
...
, any_LTR("' has no prototype")
      ...
      any _list52=PROP(statements_,PROP(body_,this));
      { var item=undefined;
      for(int item__inx=0 ; item__inx<_list52.value.arr->length ; item__inx++){item=ITEM(item__inx,_list52);
...
//now check each accessor
        ...
        //for each ac in .accessors
        any _list56=PROP(accessors_,this);
        { var ac=undefined;
        for(int ac__inx=0 ; ac__inx<_list56.value.arr->length ; ac__inx++){ac=ITEM(ac__inx,_list56);
...
//else: declare (name affinity|var) (VariableDecl,)
      //else if .specifier in ['affinity','var']
          ...
          //for each varDecl in .names
          any _list58=PROP(names_,this);
          { var varDecl=undefined;
          for(int varDecl__inx=0 ; varDecl__inx<_list58.value.arr->length ; varDecl__inx++){varDecl=ITEM(varDecl__inx,_list58);
...
, opt
            ...
            any _list59=PROP(accessors_,PROP(varRef_,this));
            { var ac=undefined;
...
//### helper function AddGlobalClasses()
    any Validate_AddGlobalClasses(DEFAULT_ARGUMENTS){
        ...
        any _list60=_newArray(argc,arguments);
        { var name=undefined;
        for(int name__inx=0 ; name__inx<_list60.value.arr->length ; name__inx++){name=ITEM(name__inx,_list60);
