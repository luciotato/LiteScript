   ...
   var Grammar = require('./Grammar');
    ...
    ASTBase.prototype.getOwnerPrefix = function(){
     ...
     Grammar.ForEachInArray.prototype.produceInMap = function(iterable){
         ...
         this.out("var ", this.mainVar.name, "=undefined;", NL);
         this.out('if(!', iterable, '.map_members) throw(new Error("for each in map: not a Map, no .map_members"));', NL);
         this.out("for ( var ", indexVarName, " in ", iterable, ".map_members)");
         this.out(" if (", iterable, ".map_members.hasOwnProperty(", indexVarName, "))");
         if (this.mainVar) {
             this.body.out("{", this.mainVar.name, "=", iterable, ".map_members[", indexVarName, "];", NL);
         };
         this.out(this.where);
         this.body.out("{", this.body, "}", NL);
         if (this.mainVar) {
     ...
     Grammar.ForIndexNumeric.prototype.produce = function(iterable){
       ...
       if (this.conditionPrefix === 'to') {
           ...
           if (this.increment) {this.sayErr("Do not use 'for... to' and a increment statement. Use 'while|until'");};
       }
       else {
       ...
       this.out("; ");
       if (this.increment) {
           this.out({CSL: this.increment.statements});
       }
       else {
           this.out(this.indexVar.name, "++");
       };
       this.out(") ", this.where);
    ...
    Grammar.FunctionDeclaration.prototype.produce = function(theProperties){
     ...
     Grammar.CaseWhenExpression.prototype.produce = function(){
       if (this.varRef) {
           ...
           this.out("        }.call(this,", this.varRef, "))");
       }
       else {
         for( var caseWhenSection__inx=0,caseWhenSection ; caseWhenSection__inx<this.cases.length ; caseWhenSection__inx++){caseWhenSection=this.cases[caseWhenSection__inx];
