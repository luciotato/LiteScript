    ...
    //    append to class Grammar.IfStatement ###
      ...
      Grammar.IfStatement.prototype.produce = function(){
        ...
        if (this.body instanceof Grammar.SingleLineBody) {
            ...
            //.out "if ", .conditional,' Then ',.body,':End If'
            this.out("if ", this.conditional, ' Then ', this.body, ':End If');
        }
        //if .body instanceof Grammar.SingleLineBody
...
//When Map is implemented using js "Object"
      ...
      Grammar.ForEachInArray.prototype.produceInMap = function(iterable){
          ...
          //.body.out indexVarName, "=item.key",NL
          this.body.out(indexVarName, "=item.key", NL);
          //if .valueVar
          if (this.valueVar) {
          
              //.body.out .valueVar.name,"=item.value",NL
              this.body.out(this.valueVar.name, "=item.value", NL);
          };
          //.out .where
          this.out(this.where);
          //.out .body
          ...
          //.out "Next",{COMMENT:" for each property"},NL
          this.out("Next", {COMMENT: " for each property"}, NL);
      };
...
//`generator` prefix marks a 'generator' function that can be paused by `yield` (js/ES6 function*)
     ...
     Grammar.FunctionDeclaration.prototype.produce = function(prefix){
      ...
      if (this instanceof Grammar.MethodDeclaration) {
          ...
          this.out("Public Function ", prefix, this.name, " ", generatorMark);
      }
      //if this instance of Grammar.MethodDeclaration
...
//we need to get the class constructor, and separate other class items.
        ...
        if (this.varRefSuper) {
          ...
          //.out "Inherits ",.varRefSuper
          this.out("Inherits ", this.varRefSuper);
        };
...
//js: function-constructor-class-namespace-object (All-in-one)
        ...
        if (this.constructorDeclaration) {
            ...
            //.out "function New(", .constructorDeclaration.paramsDeclarations, ")", NL // .getEOLComment()
            this.out("function New(", this.constructorDeclaration.paramsDeclarations, ")", NL);
        }
        //if .constructorDeclaration //there was a constructor body, add specified params
        
        else {
            //.out "function New() 'default constructor",NL
            this.out("function New() 'default constructor", NL);
        };
...
//call super-class __init
        ...
        if (this.varRefSuper) {
              ...
              //.out
                  ...
                  //"    MyBase.New("
              //if .constructorDeclaration.paramsDeclarations
              this.out({COMMENT: ["default constructor: call super.constructor"]}, NL, "    MyBase.New(");
              //if .constructorDeclaration.paramsDeclarations
              if (this.constructorDeclaration.paramsDeclarations) {
...
//initialize own properties
        ...
        for( var item__inx=0,item ; item__inx<this.body.statements.length ; item__inx++){item=this.body.statements[item__inx];
          if(item.specific.constructor === Grammar.PropertiesDeclaration){
            ...
            //item.specific.produce('Me.') //produce property assignments
            item.specific.produce('Me.');
        }};// end for each in this.body.statements
...
//if we have a varRef, is a case over a value
        ...
        for( var index=0,whenSection ; index<this.cases.length ; index++){whenSection=this.cases[index];
            ...
            //.out 'case ', {CSL:whenSection.expressions,  separator:',', freeForm:1},NL
            this.out('case ', {CSL: whenSection.expressions, separator: ',', freeForm: 1}, NL);
