// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
// This js code should work on node and the browser. For node-specific code see main.js
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------

// source text

var source = []; // pre-processed, each item is a object "new Leido"

var pos = { //position on source
    lineIndex : -1
    ,originalSource_lineIndex: -1
};

// to export
var lite = {
    setSource:setSource
    ,process: process // process source
    ,advanceHook: null //optional callback
    ,sourceLines: [] //pre-processed source lines
    ,result: [] // js output
    ,addSugar:addSugar //fn to push to extraSugar_fns
    ,extraSugar_fns:[] //optional extra-sugar functions(firstword,leido,state,newState) where this=lite
    //handy fns
    ,peekNextItem:peekNextItem
    ,getSubBlock:getSubBlock 
    ,split$expressions:split$expressions // split$expressions(q, err_fn){ //returns { items:[{pre,$pos,$len,expr},...], post:string}
};


function addSugar(fn){
    lite.extraSugar_fns.push(fn);
}

/*

  Introduction:
  =============

  The Space Shuttle and the Horse's Rear End
  http://www.astrodigital.org/space/stshorse.html

  To semicolon or not to semicolon:
  ---------------------------------

    This is another engineer decision they made:
    --------------------------------------------

        - Either you choose: 1) consider line endings (EOL) as statement's end, or
        2) you choose to define a "terminator" char, and get a (semi) "free form" mode where the EOL chars are
        considered whithespace.

        K&R choose the free-form and the semiconlon as the statement terminator, since then, you're seeing
        a lot of semicolons.

        Here, again, the 72-col punchcard constraint was important in order to make the free-form preferable
        over the EOL as terminator. Also, 72-col lines make the "indent is a block" idea unfeasible.

        Today, w/o punchcards, there are more advantages into using indentation as block separator and
        using EOL as statement separator (with a line continuation escape code).

        With indentation as block separator, you're forced to properly indentate. -that's good-; you're
        doing it anyhow (if you're not insane).
        Using EOL as statement separator, you'll stop worrying about semicolons, -that's good too-.

        Lite uses EOL as statement separator and identation as block separator, but "Lite is not a language"
        so, what we do?... we do simple things:
            1. We insert a "{" when the code indents and a "}" when it's de-indented.
            2. Inside a block, we insert a semicolon at every EOL (after concating lines ending in ' _')

    Block end cues for the casual reader
    -------------------------------------

        There is another problem with curly braces and structured programming: it's called: "closing braces wtf".
        Picture the following "screen" in C (and pascal, js, java, etc)

        +-------------------------------------------
        |987|                   inx++;
        |988|               }
        |989|               importantVar=false;
        |990|            }
        |991|        }
        |992|    }
        |993|};
        |994|
        |995|
        |996|function theSecond(){
        |997|

        Let's say you were following the 500-line code of the function "theFirst", and you get distracted by a call.
        Now, you see a well-indented cascade of closing curly braces... wtf are each one of them closing?
        Now, you depend of some crazy jumping courtesy of your favorite editor's  "find matching brace" function
        (if you're lucky enough to be using your editor, and not, let's say, reading code on the web).

        In this case, you gain nothing by using indent as block separator (picture the same, but w/o the curlys)
        It's even worst, you can't be sure of how many blocks are closing.

        So, to ease code reading, it's good to have (optional) end block statements, as in...

        +------------------------------
        |987|                  inx++;
        |988|               end if // if anotherVar was 3
        |989|               importantVar=false;
        |990|           end if
        |991|
        |992|       end loop (cols)
        |993|    end loop (rows)
        |994|end function theFirst
        |995|
        |996|function theSecond(){
        |997|

        That's clearer, and easier to read.


LitesScript Laws:
=================

    -- LiteScript is not a language.

    -- If you can avoid it, do no harm.

Corollaries:
------------

    -- if your LiteScript is harder to read than pure-js, you're doing it wrong.

*/


// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
// Set pos, reset line position and indent
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
function setSource(sourceText){

    lite.sourceLines = sourceText.split("\n");

    preProcess();

    lite.result = [];
    pos.lineIndex = -1;
    pos.originalSource_lineIndex = -1;
}

// ------------------------------------------------------------------------------------------------
function process(sourceText) {

    setSource(sourceText);
    
    while (pos.lineIndex<lite.sourceLines.length) {
        processBlock(new State('main',0)); //global/module level block
    }

}

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
// LiteScript Processor
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------

//-----------------------------------------
//--- *RECURSIVE* fn processBlock ---------
//-----------------------------------------
//--- Read lines, split words, replace ----
//-----------------------------------------

/*
Key to understand LiteScript process:
    The 'processBlock' function is called recursively on each indentation increment (a new block)
    or when the line's first word is a sugary block start keyword.

    The recursion pushes each new 'state' parameter in the stack --> the process will
    enter a new state on each block, and will recover the previous state when exiting the block (return).

Clave para entender el proceso LiteScript:
    La function 'processBlock' se llama recursivmente en cada increment of indentation (block)
    o cuando la primer linea del bloque es una keyword de comienzo de un bloque 'azucarado'

    La llamada recursiva pushes el 'state' parameter en el stack --> el proceso
    entrara en un nuevo estado en cada bloque, y recuperara el estado previo al salir del bloque (return).

---------------------------
KEYWORDS (1st word on line)

    -------------------
    KEYWORD 'if'

    if a is 2 then
        console.log("a is 2")

    -> js:

    if(a==2){
        console.log("a is 2");
    }

    ---------------------------------------------------------
    KEYWORD 'case' - implmented as ANSI SQL "CASE"

        ------------------------
        -Mode 1 : CASE [var] WHEN [value]

        case [var]
            when [value] then [commands]

            when [value],[value] or [value] then
                [commands]
                [commands]
                [commands]

            when [value],[value] then
                [commands]

            else
                [commands]

        end case

        ------------------------
        -> js:

        if ([var]==value || var==value) {
            [commands];

        } else if ([var]==value || var==value || var==value) {
            [commands]
            [commands]
            [commands]

        } else {
            [commands]
        };

------------------------
        -Mode 2 : CASE 
                    WHEN boolean expressions

        case 
            when [boolean expression 1] then [commands]

            when [boolean expression 2] then
                [commands]
                [commands]
                [commands]

            when [boolean expression 3] 
                [commands]
                [commands]

            else
                [commands]

        end case


        ------------------------
        -> js:

        if ([boolean expression 1]) {
            [commands];

        } else if ([boolean expression 2]) {
            [commands]
            [commands]
            [commands]

        } else if ([boolean expression 3]) {
            [commands]

        } else {
            [commands]
        };

*/

function processBlock(state) { //uses globals: pos

    state.blockLinesCount=0;

    // default values | valores default
    if (!state.blockIndent) state.blockIndent=state.blockIntroIndent+4;
    if (!state.terminator) state.terminator="}";
    if (!state.endWord) state.endWord="end";
    if (!state.EndBlock_callback) state.EndBlock_callback = Default_EndBlock_callback;
    if (!state.separator) state.separator = ';'; // default, statement separator

    // read lines until end of block
    while(true) {

        state.leido = readNext();
        var leido = state.leido; //convenience
        if (leido.EOF) return; //EOF

        state.blockLinesCount++;
    
        //debug breakpoint
        if ( leido.line.indexOf("end for")>=0 )
            null; 
        //--------

        // check first word, for sugar code
        var firstWord = leido.nextCode();
        
        // prepare base new state 
        var newState = new State(firstWord, leido.indent);
        newState.name = leido.line;
 
        if (firstWord=="function") {

            sugar_FUNCTION(state, newState);

        } else if (firstWord=="if" || firstWord=="while") {

            sugar_BOOL_EXPRESION(state, newState);

        } else if (firstWord=="case" && state.keyword!='switch') {

            sugar_CASE(state, newState);

        } else if (firstWord=='when') {
            if (state.keyword=='case' ) {
                sugar_CASE_WHEN_subBlock(state, newState);
            }
            else {
                sugar_CASE_WHEN_DONE(state, newState);
            }

        } else if (firstWord=='for' ) {

            sugar_FOR(state, newState);

        } else if (firstWord=='else' ) {
            
            if (leido.nextCode()=='if') {

                sugar_BOOL_EXPRESION(state, newState);
            }                        

        };

        // call registered extra sugar functions
        lite.extraSugar_fns.forEach(function(extraSugar_fn){ extraSugar_fn.call(lite,state,newState)} );

        // after the sugar, depending on the indentation of the next line,
        // we can close the current block, continue, or open a sub-block.
        // peek ahead, next line
        var nextItem = peekNextItem(); // peek Next code Line
    
        if ( nextItem.indent > state.leido.indent ) { // next line is indented
            //--> this line is a block starter

            //new block
            newState.blockIndent = nextItem.indent; //indent: as first line
            if (firstWord=='var') newState.separator=','; // for 'var' blocks => ',' is item separator

            if (state.keyword=='switch') { // for js 60's "C" inherited 'switch', no open brace, no terminator
                newState.terminator=' ';
            }
            else { // regular constructions
                // add block's open brace to this line, if it's not there
                if (!newState.openBraceInserted) newState.openBraceInserted = leido.addTerminator('{',' '); 
            }

            leido.out();  // out read (& converted) block starter line
    
            // process new block
            //------------
            processBlock(newState); // RECURSIVE - sub-block
            //------------
            
            state.blockLinesCount+=newState.blockLinesCount; // add sub block lines count to this block

            nextItem = peekNextItem(); // re-peek Next code Line, after sub-block

        } 
        else { // this is another line in the block
            // add ";" if it's required
            if (!nextItem.avoidSeparator()) leido.addTerminator(state.separator); 
            leido.out();  //out read (& converted) line
        }
    
        //check for block closing
        if ( nextItem.indent < leido.indent ) { // next line is -less- indented

            // close this block, add '}'

            if (!nextItem.line.startsWith(state.terminator)) { // add '}', if it isn't there
    
                if ( nextItem.line.startsWith(state.endWord+' ') // --> end if, end function, end case, end for, etc.
                     && nextItem.indent==state.blockStarterIndent ) { // && correctly positioned
                    
                    nextItem.line = state.terminator + ' //'+nextItem.line; // close block, with optional cues as comment

                } else {
                    // no closing brace, no cues --> add closing block brace, auto-cues
                    // closing block brace on its own line
                    out( state.blockStarterIndent, state.terminator 
                                 + (state.blockLinesCount>4 && state.keyword!='var'? "// end "+state.name : "") );
                }
    
            }
    
            state.EndBlock_callback(state, nextItem); //end of block, callback hook

            break; //*** end of this block (break loop, return)
        }

        // hook for online test
        if (lite.advanceHook) lite.advanceHook();

    } //end while, loop until end of block

}


//-----------------------------------------
//-----------------------------------------
function sugar_FUNCTION(state, newState) {

    /*
    Lite: ------------------

        function now
            return today();

        function aFunction( a:number, b:string )
            var s
            for n in 1..number
                s+=b
            return s

    js: -------------------

        function now(){
            return today();
        }

        function aFunction(a,b){ //a:number, b:string
            var s;
            for(var n=0;n<number;n++){
                s+=b;
            }
            return s;
        };

    */
    var leido = state.leido;
    var words = leido.words;
    newState.functionName = leido.nextCode();
    if (leido.nextCode() !='(') { // has no ( )
        leido.appendCode('()'); // inject "()"
    }

    // remove optional type information
    var typeInfo=[];
    var lastcode = leido.code;
    
    while (leido.nextCode()) {

        if (leido.code==':') { // optional type information (a:string, b:number, c:array)
            var colon_inx = leido.inx;
            typeInfo.push(lastcode +': '+ leido.nextCode()); // save info
            words[colon_inx]=""; //clear
            words[leido.inx]=""; //clear
        }

        lastcode = leido.code;
    }

    if (typeInfo.length) words.push("// "+typeInfo.join(', ')); // add typeinfo as comment

}

//-----------------------------------------
function sugar_FOR(state, newState) {

    /*
    Lite: ------------------

        for var n=0 to x 
            [command]
            [command]
            

    js: -------------------

        for (var n=0;n<=x;n++) {
            [command]
        }

    Lite: ------------------ 'for each' every element in array -->js: for (var i=0;i<words.length;i++)

        for each item in array
            [command]
            [command]
            

    js: -------------------
    
        for (var i=0;i<words.length;i++) { var item=array[i]
            [command]
            [command]
        }
    */

    var leido = state.leido;
    var words = leido.words;

    if (leido.nextCode()!='(') { // is not pure-js 'for('... add sugar
        
        var varname;
        
        if (leido.code=='each') { // sugar for each
        
            out("//"+leido.line); // for each a in items... as comment
            varname=leido.nextCode(); // get varname
            if (leido.nextCode()!='in') state.err ("Expected 'in'. Syntax: for each x in array" );
            var arrayName = leido.nextCode();

            // modo standard
            /*out('for(var i=0;i<'+arrayName+'.length;i++){');
            words[0]= space(4)+'var '+varname+'='+arrayName+'[i];';
            words.splice(1);
            newState.openBraceInserted=true;
            */
            // Modo nuevo Array.forEach
            words[0]= arrayName+'.forEach(function('+varname+',i){';
            words.splice(1);
            newState.terminator="})";


        } else { // sugar for ... to
            
            leido.insertCode('('); // add '('
            
            if (leido.code=='var') leido.nextCode(); //read another
            varname=leido.code; // get varname
            
            var inx_eq = varname.indexOf('=');
            if (inx_eq<0) state.err ("Equal (=) not found. Expected for [var] x =... " );
            varname=varname.slice(0,inx_eq);

            var inx_to = words.indexOf('to');
            if (inx_to<0) state.err ("Expected 'to' after 'for'");
            
            words[inx_to]=';'+varname+'<='; // add ;varname<=x
            leido.appendCode(';'+varname+'++)') // add ;varname++)
        }
    }
}

//-----------------------------------------
//-----------------------------------------
function sugar_BOOL_EXPRESION(state, newState) {

    var thenKeywordFound=false;

    var insertedOpeningPar = false;

    var leido = state.leido;

    var prev_inx = leido.inx;
    
    var words = leido.words; //convenience
 
    var codecount=0;
    
    // process 'if'/'while' line words
    while (leido.nextCode() && !thenKeywordFound) {
    
        codecount++;
        if (codecount==1 && leido.code!='(') { // missing "("
            insertedOpeningPar = leido.insertCode('('); // returns true if inserted 
        }
            
        switch(leido.code) {

            case "not": case "no": leido.replaceCode("!"); break;

            case "<>": leido.replaceCode("!="); break;

            case "and": leido.replaceCode("&&"); break;

            case "or": leido.replaceCode("||"); break;

            case "then": 
                leido.replaceCode( insertedOpeningPar? ")": "" );
                thenKeywordFound=true;
                break;

            case "is":
                var is_inx = leido.inx;
                leido.replaceCode("==");

                if (!leido.nextCode()) state.err("error parsing keyword 'is'. Expected: more words after 'is'");

                var negated=false;
                if (leido.code=='not'){
                    words[is_inx]="!="; // <-- replacement for 'is not'
                    negated=true;
                    leido.replaceCode(""); //remove ' not'
                    if (!leido.nextCode()) state.err("error parsing keyword 'is not'. Expected: another word after 'is not'");
                }

                // basic replacement:  is -> ==, is not -> !=
                // special cases: x is function, is string, is array, is object  -> (typeof x === 'type')
                //                x is null, x is undefined -> if (!x) {...

                // Usage 1: is ->  ==
                // Usage 2: [variable] is [null|undefined]  ->js:  if (![variable]){...   // false for null,undefined and ''
                // Usage 3: [variable] is [function|string|object|array]  ->js:  typeof [variable]==='[keyword]'"

                switch(leido.code) { //special cases

                    case "function": case "string": case "number": //-> (typeof x ==='type')
                    case "object": case "Object":
                        words[prev_inx]="typeof "+ words[prev_inx]; // [var] --> typeof [var]
                        words[is_inx] += "=";  // is --> '===' 
                        words[leido.inx]= "'" + words[leido.inx] + "'"; // quote keyword function|string|object...
                        break;

                     case "array": case "Array": // --> Object.prototype.toString.call(x)==='[object Array]'
                        words[prev_inx]="Object.prototype.toString.call(" +words[prev_inx]+")";
                        words[is_inx] += "="; // is --> '===' 
                        words[leido.inx]="'[object Array]'";
                        break;

                    case "null": case "undefined":
                        words[is_inx] = words[prev_inx]; //variable
                        words[prev_inx]=(negated?'':'!'); // if x is null -> if( !x ) {...
                        words[leido.inx]=""; 
                        break;

                    } // end special cases for 'is'

                break; //case 'is'

            default:
                leido.replaceCode(leido.code.replaceAll('<>','!=')); // <> --> !=
        }

        prev_inx = leido.inx;
    
    } // each code word

    if ( !thenKeywordFound && insertedOpeningPar) { // if there was not a 'then' keyword.... | si NO HUBO un 'then'... 
                                                    // and the opening par was inserted...
        leido.appendCode(')'); // add closing par ")" | agrego ")"
    }

}

//-----------------------------------------
//-----------------------------------------
function sugar_CASE(state, newState) {

    // "case"'s first line
    
    var leido = state.leido;
    var words=leido.words; //convenience
    
    newState.keyword='case';
    newState.name=leido.line;
    
    var case_inx = leido.inx;
    
    if (!leido.nextCode()) { //only 'case', mode 1: case /n when [bool expression] then...

        words[case_inx] = "// " + words[case_inx]; //comment case


    } else { // mode 2: case [variable] /n when [value] then...

        var var_inx = leido.inx;
        newState.varName = leido.code;
        
        if (!leido.nextCode()) { // only one word, single variable
        
            words[case_inx]="{ //"+words[case_inx]; //open block, comment original code
        
        } else { // more than one word => expression, store to evaluate only once            
            out(leido.indent, "//" + leido.line); //add comment, original source
            newState.varName='$value';
            words[case_inx]="{ var " + newState.varName + '=' ; //evaluate expresion once, store in $value
            leido.addTerminator(";");
        }
            
        newState.openBraceInserted=true; //already inserted
        
    }

}

// -------------------------------
function sugar_CASE_WHEN_subBlock(state, newState) {

    //check
    if ( state.keyword!='case') state.err("'when' without 'case'");

    var leido = state.leido;

    newState.keyword='when';
    newState.name=leido.line;

    leido.replaceCode("if");
    if (state.blockLinesCount>1) leido.insertCode('else ');

    if (state.varName) {

        leido.replaceCode("if (");
        
        sugar_CASE_VAR_subBlock(state, newState);
    
    } else {

        sugar_BOOL_EXPRESION(state, newState);

    }
}


// -------------------------------
function sugar_CASE_WHEN_DONE(state, newState) {
    
    var leido = state.leido;
    leido.replaceCode(',');// replace 'when'

    //check
    if (leido.nextCode()!='done') state.err("found 'when' expected 'done:'");
    leido.replaceCode('');// replace 'done'

    if (leido.nextCode()==':') { leido.replaceCode('');leido.nextCode(); } // optional ":"

    if (!leido.code) { // when done: /n --> start subblock, anonymous function
        //replace
        leido.words=[', function()'];
        return; //continue, process subblock
    }
    else {
        if (leido.code=='function') return; //leave it as it is
        // specific callback
        // strip parameters, if added
        if (leido.nextCode()=="(") leido.insertCode('//'); // comment rest of line, function params
    }
}

// -------------------------------
function sugar_CASE_VAR_subBlock(state, newState) {

    var leido = state.leido;
    var words=leido.words; //convenience

    var valueCount=0;
    var thenFound=false;
    var codeFound=false;
    var orAdded=false;
    
    // check each word in the 'when' line (to construct js's condition $value=value1 ||...)
    // veo cada palabra en esta linea de "when" (valores para js's condition)
    while (leido.nextCode()) {

        if (leido.code==':' ) {
            words[leido.inx]=""; //  optionals, delete | opcionales, borro
                
        } else if (leido.code==',' || leido.code=='or' ) {
            words[leido.inx]="||"; //  or value==...
            orAdded=true;

        } else if (leido.code=='then') { // 'then' -> //end of value list | fin de lista de valores 
            thenFound=true;
            var then_inx=leido.inx;
            words[then_inx]=")"; // replace 'then'
            if (leido.nextCode()) { 
                // there is code after 'when', assume single line 'when'
                words[then_inx]=") {"; // replace'then'
                codeFound = true;
                leido.addTerminator('}');
                break; //end of value list
            }
        }
        else { // a value
            var compare='==';
            if (words[leido.inx]=='""' || words[leido.inx]=="''" || words[leido.inx]=="0" ) compare='==='; // avoid cloud9 warnings
            if (++valueCount>1 && !orAdded) leido.insertCode('||');
            leido.replaceCode(state.varName + compare + words[leido.inx]); // Lite: 'when [value]' --> js: '...|| $value==[value]'
            orAdded=false;
        } 

    }//end while

    if (!thenFound)  {
        leido.appendCode(")");
    }

    //check, in case there is no code after 'case'
    if (!codeFound)  {
        var nextItem=peekNextItem();
        if (nextItem.indent <= state.blockIndent) state.err("expected some CODE for the 'case/when' block");
    }

}


// -------------------------------


//----------------------------------
function closeBlock(state){  // add '}', call callback

    var nextItem = peekNextItem(); // peek Next code Line

    if (nextItem && !nextItem.line.startsWith(state.terminator)) { // add '}', if it isn't there

        if ( nextItem.line.startsWith(state.endWord+' ') // --> end if, end function, end case, end for, etc.
             && nextItem.indent==state.blockStarterIndent ) { // correctly positioned
            
            nextItem.line = state.terminator + ' //'+nextItem.line; // close block, with optional cues as comment
            
        } else {
            // no closing brace, no cues --> add closing block brace, auto-cues
            // closing block brace on its own line
            out( state.blockStarterIndent, state.terminator 
                         + (state.blockLinesCount>4 && state.keyword!='var'? "// end "+state.name : "") );
        }

    }

    state.EndBlock_callback(state, nextItem); //end of block, callback hook
}


//-----------------------------------------
function Default_EndBlock_callback(state, nextItem) {
    // end of block hook
    return null;
}


//----------------------------
// utility protoype functions
//----------------------------
String.prototype.startsWith =
    function(s){ return this.slice(0, s.length) == s; };

String.prototype.isComment =
    function() { return (this.length>1 && ( this.slice(0,2)=="//" || this.slice(0,2)=="/*" ) ); };

String.prototype.isCode =
    function() { return (this.length>0 && this!=' ' && !this.isComment() ); };

String.prototype.endsWith =
    function(s){ return this.slice(-s.length) == s; };

String.prototype.replaceAt =
    function(index, count, replacement) { return this.slice(0, index) + replacement + this.slice(index + count); };

String.prototype.isEmpty =
    function() { return (!this || this.length===0 || this.trimLeft().trimRight().length===0); };

String.prototype.replaceAll = function (find, replace) {
    return this.replace(new RegExp(find,'ig'), replace);
}

//----------------------------
// utility functions
//----------------------------

function space(cant){
    var spaces=[];
    spaces[cant]='';
    return spaces.join(' ');
}


// -------------------------
function queueLine(line)
{
    if (line.trimLeft().trimRight()) {// si la linea tiene algo... | if there's something...

        if (pos.lineIndex>source.length) pos.lineIndex = source.length;

        source.splice(pos.lineIndex,0, new Leido(line)); //insert
    }
}

//----------------------------
// class Leido
//
//- 'new' Constructor
function Leido(line) {

    if ( ! this instanceof Leido) return new Leido(line); // in case somebody forgets the "new"

    //analyze line --> { indent:number,  line:string, hasCode:bool}

    // Count & Remove leading spaces
    var indent=0;
    for (var n = 0; n < line.length; n++) {
        if (line[n] === " " ) indent++;
        else if ( line[n] === "\t") indent+=4;
        else break; //not space or tab
    }

    line = line.slice(n); //cut leading spaces/tabs
    
    this.line = line;
    this.indent = indent;

    this.lineComment 
        = (line.length>1 && 
            ( line.slice(0,2)=="//" || line.slice(0,2)=="/*" )    
        );
    
    this.hasCode = ((line && !this.lineComment)? true: false);

    this.inx=-1;        
}

//- Methods

Leido.prototype.EOL = function() { 
    return (this.inx>=this.words.length);
}

Leido.prototype.readNextCode = function() { 
    this.code ='';
    this.inx++;
    if (!this.words) return false; //no words
    // skip spaces & comments
    while (this.inx<this.words.length && !this.words[this.inx].isCode()) this.inx++;
    if (this.inx>=this.words.length) return false; // ***** no more code words
    this.code = this.words[this.inx];
    return true;
}

Leido.prototype.nextCode = function() { 
    this.readNextCode(); //get next
    return this.code; //return it
}

Leido.prototype.replaceCode = function(s) { 
    if (this.inx<this.words.length) this.words[this.inx] = s;
}

Leido.prototype.avoidSeparator = function() { 
    return this.line.startsWith('when') || this.line.startsWith('{');
}
    

// -------------------------------
Leido.prototype.insertCode = function(char){

    var inx= this.inx;
    // adds "(" if it isn't there

    // skip comments and spaces
    var n=inx;
    while ( n<this.words.length
            && (this.words[n]==' ' || this.words[n].isComment() )
          ) 
       n++;

    // add '(' if it isn't there
    if (n>=this.words.length || !this.words[n].startsWith(char)) { // next word does not have it
        this.words.splice(inx,0,char); // insert at inx
        if (inx<=this.inx) this.inx++; //adjust leido.inx
        return true; //inserted
    }
 
    return false;        
}

//----------------------------
Leido.prototype.appendCode = function(char){

    // adds ")" to the end of the line (considering comments)

    // skip comments and spaces at the end of the line
    var n=this.words.length-1;
    while ( n>=0 && (this.words[n]==' ' || this.words[n].isComment()) )
       n--;

    // insert after
    this.words.splice(n+1,0,char); 
}

//----------------------------
Leido.prototype.addTerminator = function(terminator, pre){

    // adds ";" or " {" to the end of the line (considering comments)

    if (!this.commentLine) {

        // default value
        pre=pre||''; 
    
        // skip comments and spaces at the end of the line
        var n=this.words.length-1;
        while ( n>=0 && (this.words[n]==' ' || this.words[n].isComment()) )
           n--;

        // add ';' or '{' if it isn't there
        if (n==-1) { // only comments
            this.words.unshift(pre + terminator); // insert at 0
            return true; // inserted
        }
        else
            if (!this.words[n].endsWith(terminator) && !(terminator==';' && this.words[n]=='}') ) {
                this.words[n] += (pre + terminator);
                return true; // inserted
            }

        return false; // not inserted
    }

}

//----------------------------
Leido.prototype.out = function(){

    // output result | saco resultado
    out( this.indent, this.words.join('').trimRight() )

}

// end class Leido
//----------------------------


//----------------------------
// class State
//
//- 'new' Constructor
function State(keyword,indent) {
    if ( ! this instanceof State) return new State(keyword,indent); // in case somebody forgets the "new"
    this.keyword=keyword;
    this.blockStarterIndent=indent; 
    this.blockIndent=indent+4; //default
}
// -------------------------
State.prototype.err = function(s) {
    console.log("line: " + lite.result.length + ", ERR: " + s);
    out(this.blockIndent, "---------------------------------------------------");
    out(this.blockIndent, "!!! ERROR: " + s);
    out(this.blockIndent, "source line " + this.leido.sourceLinesStart+":");
    out(this.leido.indent, this.leido.line);
    out(this.blockIndent, "---------------------------------------------------");
}

// end class State
//----------------------------
    

//----------------------------
function out(indent, s){
    // agrego linea al resultado
    lite.result.push( space(indent) + s );
}

//----------------------------
function preProcess() {// sourceLines[ string ] --> source[ {hasCode:bool, EOF:bool, indent:number, line:string },... ]

    source=[];
    
    var insideMultilineComment=false;
    var isHTML=false;

    var leido;
    
    //pre-process all source lines
    //append on continuation code
    for(var n=0; n<lite.sourceLines.length; n++) {

        var start = n;
        var line = lite.sourceLines[n].trimRight(); //trim extra spaces

        // line continuation code: ' _'
        while (n < lite.sourceLines.length-1 && line.length>=2 && line.slice(-2)==' _') {
            line=line.slice(0,-2) + lite.sourceLines[++n].trimLeft().trimRight(); //append next line
        }

        leido = new Leido(line);
        
        line = leido.line; // convenience. w/o leading spaces
        
        leido.sourceLinesStart = start;
        leido.sourceLinesEnd = n;

        if (line.startsWith("/*" )) insideMultilineComment= true;
        if (!insideMultilineComment && line.startsWith('<')) isHTML = true;
        
        if (insideMultilineComment || isHTML) { 
            leido.commentLine = true;
            leido.hasCode = false;
        }
        
        if (insideMultilineComment && line.indexOf("*/")>=0) {
            insideMultilineComment= false;
        }

        if (!insideMultilineComment && isHTML && line.indexOf("<script>")>=0) {
            isHTML= false;
        }

        source.push(leido);
    }

    //add end line, close all blocks, indent=-1
    leido = new Leido('');
    leido.indent=-1;
    leido.EOF=true;
    leido.words=[];
    source.push( leido );

    //reset indexes
    pos.lineIndex=-1;
    pos.originalSource_lineIndex=0;

}

//----------------------------
function peekNextItem() {//peek ahead next code line

    for(var n=pos.lineIndex+1; n<source.length-1; n++) {
        if (source[n].hasCode) return source[n]; //another line
    }
    return source[n]; //return last extra line, indent=-1
}


// ------------------------------------------------------------------------------------------------
function splitJSLine(s){
    var start=0;
    var previous;
    var quoteChar;
    var inQuotes = false;
    var res=[];

    for(var i=0;i<s.length;i++) {

        var char = s[i];

        if (inQuotes){
            if (char==quoteChar && i<s.length && s[i+1]!=char){ //end of quotes
                inQuotes=false;
                var quoted=s.slice(start,i+1);
                if (quoteChar=='"') quoted = replace$inPlace(quoted);
                res.push(quoted );
                start = i+1;
            }
            else null; //do nothing, still in quotes
        }
        else { // not in quotes

            switch (char) {

                case '"': case "'": // start quotes
                    if (start<i) res.push(s.slice(start,i)); // si habia algo antes de las comillas
                    start=i;
                    inQuotes=true;
                    quoteChar=char;
                    break;

                case '/':
                    if (previous=='/'){ // //-comment
                        res.push(s.slice(start));
                        i=s.length;
                        start=i+1;
                    }
                    break;

                // Separators
                case ' ': case ':': case ',': case ';': case '(': case ')': case '{': case '}': case '[': case ']':
                    if (start<i) res.push(s.slice(start,i)); // si habia algo antes del separ
                    res.push(char);
                    start = i+1;
                    break;

            } // end switch

        } // end if

        previous=char;
    } // end for

    if (start<i) res.push(s.slice(start,i)); // last word

    //console.log(res);
    return res;
}

//----------------------------
function readNext() // read next source code line
                    //return leido={line:string, indent:number, words:[], [EOF:true]}
                    // skips comments and empty lines
{

    // while empty or comment lines
    // mientras sean lineas vacias o de comentario
    while(true){

        pos.lineIndex++;
        
        if (pos.lineIndex >= source.length-1) {
            return source[source.length-1]; //extra line, indent=-1, close all blocks
        }

        var leido = source[pos.lineIndex]; //advance, position on pre-processed source

        //position on original source
        if (leido.sourceLinesEnd) pos.originalSource_lineIndex=leido.sourceLinesEnd;

        if (leido.hasCode) break; // Code line, exit "skip blank and comments" loop
        
        out ( leido.indent, leido.line );
        // isn't code, output w/o change, keep reading lines

    } // loop - keep reading lines, while line is empty or comment
      // loop - sigo leyendo, mientras sea linea vacia o de comentario


    //aqui, la linea contiene codigo
    //here, the line has code

    //split words
    leido.words = splitJSLine(leido.line);

    return leido;
}


// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
// Util Functions
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
function quoted(s){
    return '"'+s+'"';
}

function find(s,searched,start){
    var inx=s.indexOf(searched,start);
    return inx<0? s.length : inx;
}

function split$expressions(q, err_fn){ //returns { items:[{pre,$pos,$len,expr},...], post:string}

    if (!q) return {items:[],post:''};

    // look for $expression or $(expression) inside a quoted (") string
    var n,i;
    var items=[];
    
    q=q.trimRight();
    
    //clear start and end quotes
    if (q[0]=='"') q.splice(0,1);
    if (q[q.length-1]=='"') q.splice(q.length-1,1);
        
    
    var last_delimiter=-1;
    while (true) {
        var item={};
        n=q.indexOf('$',last_delimiter+1);
        if (n<0) break;
        
        item.pre=q.slice(last_delimiter+1,n-last_delimiter-1);
        item.$pos=n;
        var start=n+1;
        if (q.slice(start,1)=="(") { // is "$(...)"
            start++;
            i=q.findMatchingPair(start,")");
            if (i<0) {
                err_fn && err_fn('col:' + start + ', unmatched "("');
                break;
            }
        } 
        else { // is not "$("
            // look for " " or '"' or EOL, as varname delimiter
            i = Math.min( find(q,' ',n+2) , find(q,'"',n+2) ) - 1; // first delimiter
        }

        item.$len=i-n+1;
        item.expr=q.slice(start,i-start);
        items.push(item);
        last_delimiter=i; 
    }
    return {items:items, post:q.slice(last_delimiter+1)};
}

function replace$inPlace(q,err_fn){
    
    var spl=split$expressions(q);
    var arrResult=[];
    spl.items.forEach(
        function(item,index){
            if (item.pre) {
                arrResult.push(quoted(item.pre))
                arrResult.push(' + ')
            }
            arrResult.push(item.expr)
        }
    );
    if (spl.post) arrResult.push(quoted(spl.post));
    return arrResult.join('');
}

//-------------------------------------------------
// getSubBlock() //append sub-block into leido.words
//-------------------------------------------------
function getSubBlock(state) {
    // peek ahead, next line
    var nextItem=peekNextItem(); // peek Next code Line
    while ( nextItem.indent > state.blockIndent ) { // next line is indented, so this line is a block starter
        state.leido.words.append(' '); // a space and...
        state.leido.words.append(readNext().words); // next indented line
    }
}

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
//-- node or js:require
//-- exports
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
if (this.module) { // inside 'require' or node module
    module.exports = lite;
}
