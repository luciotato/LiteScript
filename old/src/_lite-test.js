...
function ProcessFiles(err,files) {
    ...
    }// end for(var i=0;i<files.length;i++){
}// end function ProcessFiles(err,files) {
...
function readNextLine_indent() {
    ...
    return line;
}// end function readNextLine_indent()
...
function processBlock( state, endWord, callback) {
    ...
    if (! state) {
        ...
        //valores default
        if (! endWord ) endWord="end";
        if (! callback ) callback = DefaultCallback_EndBlock;
    ...
    while(true) {
        ...
        { //case state
            ...
            if ( state===States.INITIAL|| state===States.INSIDE_IF) {
                processWords(words);
            }

            else if ( state===States.INSIDE_CASE|| state===States.INSIDE_CASE_BOOLEAN) {
...
function processCaseLine(state,words) {
    ...
    if (word!='when' && word!='else' ) err("'when/else' expected inside 'case' block);

    if (words.length=1 || word[1].isComment() ) err("Expected: "+(booleanMode?"bool expression":"value") {
    ...
    if (booleanMode) { // case when [bool expression] then...
        ...
        for (n=1; n<words.lenght;n++) {
            ...
            if (word='then' || word.isComment() ) break; //fin de proceso de linea

            //no era comment
            words[n]=jsCase+" "+word[n]+":"; //tomo como otro 'case [value]:'
        }// end for (n=1; n<words.lenght;n++)
        loop;
    } //end if
...
function HandleIfBlock(words) {
    ...
    for (var n=1;n<word[n].length;n++) {
        ...
        words[n]=word;
    }// end for (var n=1;n<word[n].length;n++) {
...
function countLeadingSpaces(str) {
    ...
    loop;
    return count;
}// end function countLeadingSpaces(str)
...
function DoubleOper(word, oper) {
    ...
    while(true) {
        ...
        if (secondpos>0 && secondpos=firstpos+1) {   //Ya es un doble =
                actualpos = secondpos;
        }
        else {
