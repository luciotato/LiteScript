...
function testFile(filename)	{
	...
	try{
		...
		//moduleNode = compiler.compileLines(filename,sourceLines,{debug:true, addGlobals:["expect"]});
		moduleNode = compiler.compileModule(filename,sourceLines,compilerOptions);
		compiledLines = moduleNode.getCompiledLines();
		compiledLines.push('module.exports = Tests;');
	}catch(e){
...
var use = args.value('use','u')||DEFAULT_VERSION 
var compVersion = use+'.0'
...
var compilerPath = '../liteCompiler-'+compVersion+'/Compiler';

separ();
console.error('TEST USING ',compilerPath);
...
console.error('Starting tests from dir test-'+use+'/*');

fsUtil.recurseDir('test-'+use, function(filename){
